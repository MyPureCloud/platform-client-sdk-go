...
func (o *Documentvariationanswer) SetField(field string, fieldValue interface{}) {
...
func (o Documentvariationanswer) MarshalJSON() ([]byte, error) {
...
func (o *Documentvariationanswer) UnmarshalJSON(b []byte) error {
	var DocumentvariationanswerMap map[string]interface{}
	err := json.Unmarshal(b, &DocumentvariationanswerMap)
	if err != nil {
		return err
	}
	
	if Id, ok := DocumentvariationanswerMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if dateCreatedString, ok := DocumentvariationanswerMap["dateCreated"].(string); ok {
		DateCreated, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCreatedString)
		o.DateCreated = &DateCreated
	}
	
	if dateModifiedString, ok := DocumentvariationanswerMap["dateModified"].(string); ok {
		DateModified, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateModifiedString)
		o.DateModified = &DateModified
	}
	
	if DocumentVersion, ok := DocumentvariationanswerMap["documentVersion"].(map[string]interface{}); ok {
		DocumentVersionString, _ := json.Marshal(DocumentVersion)
		json.Unmarshal(DocumentVersionString, &o.DocumentVersion)
	}
	
	if Contexts, ok := DocumentvariationanswerMap["contexts"].([]interface{}); ok {
		ContextsString, _ := json.Marshal(Contexts)
		json.Unmarshal(ContextsString, &o.Contexts)
	}
	
	if Document, ok := DocumentvariationanswerMap["document"].(map[string]interface{}); ok {
		DocumentString, _ := json.Marshal(Document)
		json.Unmarshal(DocumentString, &o.Document)
	}
	
	if Priority, ok := DocumentvariationanswerMap["priority"].(float64); ok {
		PriorityInt := int(Priority)
		o.Priority = &PriorityInt
	}
	
	if Name, ok := DocumentvariationanswerMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Body, ok := DocumentvariationanswerMap["body"].(map[string]interface{}); ok {
		BodyString, _ := json.Marshal(Body)
		json.Unmarshal(BodyString, &o.Body)
	}
	
	if SelfUri, ok := DocumentvariationanswerMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

...
func (o *Engagementfilter) SetField(field string, fieldValue interface{}) {
...
func (o Engagementfilter) MarshalJSON() ([]byte, error) {
...
func (o *Engagementfilter) UnmarshalJSON(b []byte) error {
	var EngagementfilterMap map[string]interface{}
	err := json.Unmarshal(b, &EngagementfilterMap)
	if err != nil {
		return err
	}
	
	if Operator, ok := EngagementfilterMap["operator"].(string); ok {
		o.Operator = &Operator
	}
    
	if From, ok := EngagementfilterMap["from"].(float64); ok {
		FromInt := int(From)
		o.From = &FromInt
	}
	
	if To, ok := EngagementfilterMap["to"].(float64); ok {
		ToInt := int(To)
		o.To = &ToInt
	}
	
	if Value, ok := EngagementfilterMap["value"].(float64); ok {
		ValueInt := int(Value)
		o.Value = &ValueInt
	}

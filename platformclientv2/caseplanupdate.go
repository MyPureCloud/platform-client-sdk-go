...
func (o *Caseplanupdate) SetField(field string, fieldValue interface{}) {
...
func (o Caseplanupdate) MarshalJSON() ([]byte, error) {
...
func (o *Caseplanupdate) UnmarshalJSON(b []byte) error {
	var CaseplanupdateMap map[string]interface{}
	err := json.Unmarshal(b, &CaseplanupdateMap)
	if err != nil {
		return err
	}
	
	if Name, ok := CaseplanupdateMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if DefaultDueDurationInSeconds, ok := CaseplanupdateMap["defaultDueDurationInSeconds"].(float64); ok {
	...
	if DefaultTtlSeconds, ok := CaseplanupdateMap["defaultTtlSeconds"].(float64); ok {
		DefaultTtlSecondsInt := int(DefaultTtlSeconds)
		o.DefaultTtlSeconds = &DefaultTtlSecondsInt
	}
	
	if ReferencePrefix, ok := CaseplanupdateMap["referencePrefix"].(string); ok {
		o.ReferencePrefix = &ReferencePrefix
	}
    
	if CustomerIntentId, ok := CaseplanupdateMap["customerIntentId"].(string); ok {
		o.CustomerIntentId = &CustomerIntentId
	}
    
	if Description, ok := CaseplanupdateMap["description"].(string); ok {
		o.Description = &Description
	}
    
	if DefaultCaseOwnerId, ok := CaseplanupdateMap["defaultCaseOwnerId"].(string); ok {
		o.DefaultCaseOwnerId = &DefaultCaseOwnerId
	}
    
	if DivisionId, ok := CaseplanupdateMap["divisionId"].(string); ok {

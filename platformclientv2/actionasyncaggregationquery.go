...
type Actionasyncaggregationquery struct { 
	...
	Views *[]Actionaggregationview `json:"views,omitempty"`
...
func (o *Actionasyncaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Actionasyncaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Actionasyncaggregationquery) UnmarshalJSON(b []byte) error {
	var ActionasyncaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &ActionasyncaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := ActionasyncaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := ActionasyncaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := ActionasyncaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := ActionasyncaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := ActionasyncaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := ActionasyncaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := ActionasyncaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := ActionasyncaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := ActionasyncaggregationqueryMap["alternateTimeDimension"].(string); ok {
		o.AlternateTimeDimension = &AlternateTimeDimension
	}
    
	if PageSize, ok := ActionasyncaggregationqueryMap["pageSize"].(float64); ok {

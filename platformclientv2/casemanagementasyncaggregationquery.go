...
type Casemanagementasyncaggregationquery struct { 
	...
	Filter *Casemanagementaggregatequeryfilter `json:"filter,omitempty"`
	...
	Views *[]Casemanagementaggregationview `json:"views,omitempty"`
...
func (o *Casemanagementasyncaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Casemanagementasyncaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Casemanagementasyncaggregationquery) UnmarshalJSON(b []byte) error {
	var CasemanagementasyncaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &CasemanagementasyncaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := CasemanagementasyncaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := CasemanagementasyncaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := CasemanagementasyncaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := CasemanagementasyncaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := CasemanagementasyncaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := CasemanagementasyncaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := CasemanagementasyncaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := CasemanagementasyncaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := CasemanagementasyncaggregationqueryMap["alternateTimeDimension"].(string); ok {
		o.AlternateTimeDimension = &AlternateTimeDimension
	}
    
	if PageSize, ok := CasemanagementasyncaggregationqueryMap["pageSize"].(float64); ok {

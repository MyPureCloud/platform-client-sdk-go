...
func (o *Emailroutingestablishedevent) SetField(field string, fieldValue interface{}) {
...
func (o Emailroutingestablishedevent) MarshalJSON() ([]byte, error) {
...
func (o *Emailroutingestablishedevent) UnmarshalJSON(b []byte) error {
	var EmailroutingestablishedeventMap map[string]interface{}
	err := json.Unmarshal(b, &EmailroutingestablishedeventMap)
	if err != nil {
		return err
	}
	
	if EventId, ok := EmailroutingestablishedeventMap["eventId"].(string); ok {
		o.EventId = &EventId
	}
    
	if eventDateTimeString, ok := EmailroutingestablishedeventMap["eventDateTime"].(string); ok {
		EventDateTime, _ := time.Parse("2006-01-02T15:04:05.999999Z", eventDateTimeString)
		o.EventDateTime = &EventDateTime
	}
	
	if ConversationId, ok := EmailroutingestablishedeventMap["conversationId"].(string); ok {
		o.ConversationId = &ConversationId
	}
    
	if CommunicationId, ok := EmailroutingestablishedeventMap["communicationId"].(string); ok {
		o.CommunicationId = &CommunicationId
	}
    
	if QueueId, ok := EmailroutingestablishedeventMap["queueId"].(string); ok {
		o.QueueId = &QueueId
	}
    
	if SkillIds, ok := EmailroutingestablishedeventMap["skillIds"].([]interface{}); ok {
		SkillIdsString, _ := json.Marshal(SkillIds)
		json.Unmarshal(SkillIdsString, &o.SkillIds)
	}
	
	if LanguageId, ok := EmailroutingestablishedeventMap["languageId"].(string); ok {
		o.LanguageId = &LanguageId
	}
    
	if Label, ok := EmailroutingestablishedeventMap["label"].(string); ok {
		o.Label = &Label
	}
    
	if InitialConfiguration, ok := EmailroutingestablishedeventMap["initialConfiguration"].(map[string]interface{}); ok {
	...
	if SourceConfiguration, ok := EmailroutingestablishedeventMap["sourceConfiguration"].(map[string]interface{}); ok {

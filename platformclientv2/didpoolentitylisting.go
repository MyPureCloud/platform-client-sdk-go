...
func (o *Didpoolentitylisting) SetField(field string, fieldValue interface{}) {
...
func (o *Didpoolentitylisting) UnmarshalJSON(b []byte) error {
	var DidpoolentitylistingMap map[string]interface{}
	err := json.Unmarshal(b, &DidpoolentitylistingMap)
	if err != nil {
		return err
	}
	
	if Entities, ok := DidpoolentitylistingMap["entities"].([]interface{}); ok {
		EntitiesString, _ := json.Marshal(Entities)
		json.Unmarshal(EntitiesString, &o.Entities)
	}
	
	if PageSize, ok := DidpoolentitylistingMap["pageSize"].(float64); ok {
		PageSizeInt := int(PageSize)
		o.PageSize = &PageSizeInt
	}
	
	if PageNumber, ok := DidpoolentitylistingMap["pageNumber"].(float64); ok {
		PageNumberInt := int(PageNumber)
		o.PageNumber = &PageNumberInt
	}
	
	if Total, ok := DidpoolentitylistingMap["total"].(float64); ok {
		TotalInt := int(Total)
		o.Total = &TotalInt
	}
	
	if TotalNumberOfEntities, ok := DidpoolentitylistingMap["totalNumberOfEntities"].(float64); ok {
		TotalNumberOfEntitiesInt := int(TotalNumberOfEntities)
		o.TotalNumberOfEntities = &TotalNumberOfEntitiesInt
	}
	
	if FirstUri, ok := DidpoolentitylistingMap["firstUri"].(string); ok {
		o.FirstUri = &FirstUri
	}
	...
	if SelfUri, ok := DidpoolentitylistingMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}
	...
	if PageCount, ok := DidpoolentitylistingMap["pageCount"].(float64); ok {

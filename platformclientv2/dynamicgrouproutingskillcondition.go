...
func (o *Dynamicgrouproutingskillcondition) SetField(field string, fieldValue interface{}) {
...
func (o Dynamicgrouproutingskillcondition) MarshalJSON() ([]byte, error) {
...
func (o *Dynamicgrouproutingskillcondition) UnmarshalJSON(b []byte) error {
	var DynamicgrouproutingskillconditionMap map[string]interface{}
	err := json.Unmarshal(b, &DynamicgrouproutingskillconditionMap)
	if err != nil {
		return err
	}
	
	if RoutingSkill, ok := DynamicgrouproutingskillconditionMap["routingSkill"].(string); ok {
		o.RoutingSkill = &RoutingSkill
	}
    
	if Comparator, ok := DynamicgrouproutingskillconditionMap["comparator"].(string); ok {
		o.Comparator = &Comparator
	}
    
	if Proficiency, ok := DynamicgrouproutingskillconditionMap["proficiency"].(float64); ok {
		ProficiencyInt := int(Proficiency)
		o.Proficiency = &ProficiencyInt
	}
	
	if ChildConditions, ok := DynamicgrouproutingskillconditionMap["childConditions"].([]interface{}); ok {

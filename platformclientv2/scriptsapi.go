...
// GetScriptsPublishedDivisionviewVariables invokes GET /api/v2/scripts/published/divisionviews/{scriptId}/variables
//
// Get the published variables
func (a ScriptsApi) GetScriptsPublishedDivisionviewVariables(scriptId string, input string, output string, varType string, scriptDataVersion string) (*interface{}, *APIResponse, error) {
	var httpMethod = "GET"
	// create path and map variables
	path := a.Configuration.BasePath + "/api/v2/scripts/published/divisionviews/{scriptId}/variables"
	path = strings.Replace(path, "{scriptId}", url.PathEscape(fmt.Sprintf("%v", scriptId)), -1)
	...
	if &scriptId == nil {
		// false
		return defaultReturn, nil, errors.New("Missing required parameter 'scriptId' when calling ScriptsApi->GetScriptsPublishedDivisionviewVariables")
	}

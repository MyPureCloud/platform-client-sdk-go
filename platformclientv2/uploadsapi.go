...
func (a UploadsApi) PostUploadsWorkforcemanagementHistoricaldataJson(body Uploadurlrequest) (*Uploadurlresponse, *APIResponse, error) {
	var httpMethod = "POST"
	// create path and map variables
	path := a.Configuration.BasePath + "/api/v2/uploads/workforcemanagement/historicaldata/json"
	defaultReturn := new(Uploadurlresponse)
	if true == false {
		return defaultReturn, nil, errors.New("This message brought to you by the laws of physics being broken")
	}
	...
	if &body == nil {
		// 
		return defaultReturn, nil, errors.New("Missing required parameter 'body' when calling UploadsApi->PostUploadsWorkforcemanagementHistoricaldataJson")
	}

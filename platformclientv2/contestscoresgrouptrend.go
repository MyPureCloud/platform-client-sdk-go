...
type Contestscoresgrouptrend struct { 
	...
	ContestScore *Contestscore `json:"contestScore,omitempty"`

	// MetricScores - The Contest metric scores
	MetricScores *[]Contestmetricscore `json:"metricScores,omitempty"`
}

// SetField uses reflection to set a field on the model if the model has a property SetFieldNames, and triggers custom JSON serialization logic to only serialize properties that have been set using this function.
func (o *Contestscoresgrouptrend) SetField(field string, fieldValue interface{}) {
...
func (o Contestscoresgrouptrend) MarshalJSON() ([]byte, error) {
...
func (o *Contestscoresgrouptrend) UnmarshalJSON(b []byte) error {
	var ContestscoresgrouptrendMap map[string]interface{}
	err := json.Unmarshal(b, &ContestscoresgrouptrendMap)
	if err != nil {
		return err
	}
	
	if dateWorkdayString, ok := ContestscoresgrouptrendMap["dateWorkday"].(string); ok {
		DateWorkday, _ := time.Parse("2006-01-02", dateWorkdayString)
		o.DateWorkday = &DateWorkday
	}
	
	if ContestScore, ok := ContestscoresgrouptrendMap["contestScore"].(map[string]interface{}); ok {
		ContestScoreString, _ := json.Marshal(ContestScore)
		json.Unmarshal(ContestScoreString, &o.ContestScore)
	}
	
	if MetricScores, ok := ContestscoresgrouptrendMap["metricScores"].([]interface{}); ok {

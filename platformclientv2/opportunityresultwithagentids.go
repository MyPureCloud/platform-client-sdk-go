...
func (o *Opportunityresultwithagentids) SetField(field string, fieldValue interface{}) {
...
func (o Opportunityresultwithagentids) MarshalJSON() ([]byte, error) {
...
func (o *Opportunityresultwithagentids) UnmarshalJSON(b []byte) error {
	var OpportunityresultwithagentidsMap map[string]interface{}
	err := json.Unmarshal(b, &OpportunityresultwithagentidsMap)
	if err != nil {
		return err
	}
	
	if Id, ok := OpportunityresultwithagentidsMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if startDateString, ok := OpportunityresultwithagentidsMap["startDate"].(string); ok {
		StartDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", startDateString)
		o.StartDate = &StartDate
	}
	
	if endDateString, ok := OpportunityresultwithagentidsMap["endDate"].(string); ok {
		EndDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", endDateString)
		o.EndDate = &EndDate
	}
	
	if Status, ok := OpportunityresultwithagentidsMap["status"].(string); ok {
		o.Status = &Status
	}
    
	if openDateString, ok := OpportunityresultwithagentidsMap["openDate"].(string); ok {
		OpenDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", openDateString)
		o.OpenDate = &OpenDate
	}
	
	if deadlineDateString, ok := OpportunityresultwithagentidsMap["deadlineDate"].(string); ok {
		DeadlineDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", deadlineDateString)
		o.DeadlineDate = &DeadlineDate
	}
	
	if Name, ok := OpportunityresultwithagentidsMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Description, ok := OpportunityresultwithagentidsMap["description"].(string); ok {
		o.Description = &Description
	}
    
	if ActivityCodeId, ok := OpportunityresultwithagentidsMap["activityCodeId"].(string); ok {
		o.ActivityCodeId = &ActivityCodeId
	}
    
	if ApprovalType, ok := OpportunityresultwithagentidsMap["approvalType"].(string); ok {
		o.ApprovalType = &ApprovalType
	}
    
	if AgentCount, ok := OpportunityresultwithagentidsMap["agentCount"].(float64); ok {
		AgentCountInt := int(AgentCount)
		o.AgentCount = &AgentCountInt
	}
	
	if Capacity, ok := OpportunityresultwithagentidsMap["capacity"].(float64); ok {
		CapacityInt := int(Capacity)
		o.Capacity = &CapacityInt
	}
	
	if EnrollmentProcessingCount, ok := OpportunityresultwithagentidsMap["enrollmentProcessingCount"].(float64); ok {
		EnrollmentProcessingCountInt := int(EnrollmentProcessingCount)
		o.EnrollmentProcessingCount = &EnrollmentProcessingCountInt
	}
	
	if EnrollmentCounts, ok := OpportunityresultwithagentidsMap["enrollmentCounts"].(map[string]interface{}); ok {
		EnrollmentCountsString, _ := json.Marshal(EnrollmentCounts)
		json.Unmarshal(EnrollmentCountsString, &o.EnrollmentCounts)
	}
	
	if publishedDateString, ok := OpportunityresultwithagentidsMap["publishedDate"].(string); ok {
		PublishedDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", publishedDateString)
		o.PublishedDate = &PublishedDate
	}
	
	if closedDateString, ok := OpportunityresultwithagentidsMap["closedDate"].(string); ok {
		ClosedDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", closedDateString)
		o.ClosedDate = &ClosedDate
	}
	
	if SystemMessageCode, ok := OpportunityresultwithagentidsMap["systemMessageCode"].(string); ok {
		o.SystemMessageCode = &SystemMessageCode
	}
    
	if Metadata, ok := OpportunityresultwithagentidsMap["metadata"].(map[string]interface{}); ok {
		MetadataString, _ := json.Marshal(Metadata)
		json.Unmarshal(MetadataString, &o.Metadata)
	}
	
	if AgentIds, ok := OpportunityresultwithagentidsMap["agentIds"].([]interface{}); ok {
		AgentIdsString, _ := json.Marshal(AgentIds)
		json.Unmarshal(AgentIdsString, &o.AgentIds)
	}
	
	if SelfUri, ok := OpportunityresultwithagentidsMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

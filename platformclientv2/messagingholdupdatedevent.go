...
func (o *Messagingholdupdatedevent) SetField(field string, fieldValue interface{}) {
...
func (o Messagingholdupdatedevent) MarshalJSON() ([]byte, error) {
...
func (o *Messagingholdupdatedevent) UnmarshalJSON(b []byte) error {
	var MessagingholdupdatedeventMap map[string]interface{}
	err := json.Unmarshal(b, &MessagingholdupdatedeventMap)
	if err != nil {
		return err
	}
	
	if EventId, ok := MessagingholdupdatedeventMap["eventId"].(string); ok {
		o.EventId = &EventId
	}
    
	if eventDateTimeString, ok := MessagingholdupdatedeventMap["eventDateTime"].(string); ok {
		EventDateTime, _ := time.Parse("2006-01-02T15:04:05.999999Z", eventDateTimeString)
		o.EventDateTime = &EventDateTime
	}
	
	if ConversationId, ok := MessagingholdupdatedeventMap["conversationId"].(string); ok {
		o.ConversationId = &ConversationId
	}
    
	if CommunicationId, ok := MessagingholdupdatedeventMap["communicationId"].(string); ok {
		o.CommunicationId = &CommunicationId
	}
    
	if Held, ok := MessagingholdupdatedeventMap["held"].(bool); ok {
		o.Held = &Held
	}

...
func (o *Customattributesupdaterequest) SetField(field string, fieldValue interface{}) {
...
func (o Customattributesupdaterequest) MarshalJSON() ([]byte, error) {
...
func (o *Customattributesupdaterequest) UnmarshalJSON(b []byte) error {
	var CustomattributesupdaterequestMap map[string]interface{}
	err := json.Unmarshal(b, &CustomattributesupdaterequestMap)
	if err != nil {
		return err
	}
	
	if Id, ok := CustomattributesupdaterequestMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Divisions, ok := CustomattributesupdaterequestMap["divisions"].([]interface{}); ok {
		DivisionsString, _ := json.Marshal(Divisions)
		json.Unmarshal(DivisionsString, &o.Divisions)
	}
	
	if SchemaId, ok := CustomattributesupdaterequestMap["schemaId"].(string); ok {
		o.SchemaId = &SchemaId
	}
    
	if Version, ok := CustomattributesupdaterequestMap["version"].(float64); ok {
		VersionInt := int(Version)
		o.Version = &VersionInt
	}
	
	if CustomAttributes, ok := CustomattributesupdaterequestMap["customAttributes"].(map[string]interface{}); ok {

...
type Resolutionasyncaggregationquery struct { 
	...
	Filter *Resolutionaggregatequeryfilter `json:"filter,omitempty"`
...
func (o *Resolutionasyncaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Resolutionasyncaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Resolutionasyncaggregationquery) UnmarshalJSON(b []byte) error {
	var ResolutionasyncaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &ResolutionasyncaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := ResolutionasyncaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := ResolutionasyncaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := ResolutionasyncaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := ResolutionasyncaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := ResolutionasyncaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := ResolutionasyncaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := ResolutionasyncaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := ResolutionasyncaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := ResolutionasyncaggregationqueryMap["alternateTimeDimension"].(string); ok {
		o.AlternateTimeDimension = &AlternateTimeDimension
	}
    
	if PageSize, ok := ResolutionasyncaggregationqueryMap["pageSize"].(float64); ok {

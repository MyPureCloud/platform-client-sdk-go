...
func (o *Aggregatedexportsnapshotjobrequest) SetField(field string, fieldValue interface{}) {
...
func (o Aggregatedexportsnapshotjobrequest) MarshalJSON() ([]byte, error) {
...
func (o *Aggregatedexportsnapshotjobrequest) UnmarshalJSON(b []byte) error {
	var AggregatedexportsnapshotjobrequestMap map[string]interface{}
	err := json.Unmarshal(b, &AggregatedexportsnapshotjobrequestMap)
	if err != nil {
		return err
	}
	
	if Granularity, ok := AggregatedexportsnapshotjobrequestMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := AggregatedexportsnapshotjobrequestMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if Delimiter, ok := AggregatedexportsnapshotjobrequestMap["delimiter"].(string); ok {
		o.Delimiter = &Delimiter
	}
    
	if PlanningGroupIds, ok := AggregatedexportsnapshotjobrequestMap["planningGroupIds"].([]interface{}); ok {
		PlanningGroupIdsString, _ := json.Marshal(PlanningGroupIds)
		json.Unmarshal(PlanningGroupIdsString, &o.PlanningGroupIds)
	}
	
	if dateStartString, ok := AggregatedexportsnapshotjobrequestMap["dateStart"].(string); ok {
		DateStart, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateStartString)
		o.DateStart = &DateStart
	}
	
	if dateEndString, ok := AggregatedexportsnapshotjobrequestMap["dateEnd"].(string); ok {
		DateEnd, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateEndString)
		o.DateEnd = &DateEnd
	}
	
	if SnapshotId, ok := AggregatedexportsnapshotjobrequestMap["snapshotId"].(string); ok {

...
type Longtermrequirements struct { 
	...
	ForecastMetadata *Forecastmetadata `json:"forecastMetadata,omitempty"`
...
func (o *Longtermrequirements) SetField(field string, fieldValue interface{}) {
...
func (o Longtermrequirements) MarshalJSON() ([]byte, error) {
	...
	if len(o.SetFieldNames) > 0 {
		...
		dateTimeFields := []string{ "DateGenerationStarted", }
	...
	DateGenerationStarted := new(string)
	if o.DateGenerationStarted != nil {
		
		*DateGenerationStarted = timeutil.Strftime(o.DateGenerationStarted, "%Y-%m-%dT%H:%M:%S.%fZ")
	} else {
		DateGenerationStarted = nil
	}
...
func (o *Longtermrequirements) UnmarshalJSON(b []byte) error {
	var LongtermrequirementsMap map[string]interface{}
	err := json.Unmarshal(b, &LongtermrequirementsMap)
	if err != nil {
		return err
	}
	
	if ForecastMetadata, ok := LongtermrequirementsMap["forecastMetadata"].(map[string]interface{}); ok {
		ForecastMetadataString, _ := json.Marshal(ForecastMetadata)
		json.Unmarshal(ForecastMetadataString, &o.ForecastMetadata)
	}
	...
	if RequirementResults, ok := LongtermrequirementsMap["requirementResults"].([]interface{}); ok {
		RequirementResultsString, _ := json.Marshal(RequirementResults)
		json.Unmarshal(RequirementResultsString, &o.RequirementResults)
	}

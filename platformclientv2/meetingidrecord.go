...
func (o *Meetingidrecord) SetField(field string, fieldValue interface{}) {
...
func (o Meetingidrecord) MarshalJSON() ([]byte, error) {
...
func (o *Meetingidrecord) UnmarshalJSON(b []byte) error {
	var MeetingidrecordMap map[string]interface{}
	err := json.Unmarshal(b, &MeetingidrecordMap)
	if err != nil {
		return err
	}
	
	if Id, ok := MeetingidrecordMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Ephemeral, ok := MeetingidrecordMap["ephemeral"].(bool); ok {
		o.Ephemeral = &Ephemeral
	}
    
	if ConferenceId, ok := MeetingidrecordMap["conferenceId"].(string); ok {
		o.ConferenceId = &ConferenceId
	}
    
	if dateExpiredString, ok := MeetingidrecordMap["dateExpired"].(string); ok {
		DateExpired, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateExpiredString)
		o.DateExpired = &DateExpired
	}
	
	if SelfUri, ok := MeetingidrecordMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

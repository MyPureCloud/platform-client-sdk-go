...
func (o *Queueconversationeventtopicemail) UnmarshalJSON(b []byte) error {
	...
	if QueueMediaSettings, ok := QueueconversationeventtopicemailMap["queueMediaSettings"].(map[string]interface{}); ok {
		QueueMediaSettingsString, _ := json.Marshal(QueueMediaSettings)
		json.Unmarshal(QueueMediaSettingsString, &o.QueueMediaSettings)
	}
	
	if resumeTimeString, ok := QueueconversationeventtopicemailMap["resumeTime"].(string); ok {
		ResumeTime, _ := time.Parse("2006-01-02T15:04:05.999999Z", resumeTimeString)
		o.ResumeTime = &ResumeTime
	}
	
	if parkTimeString, ok := QueueconversationeventtopicemailMap["parkTime"].(string); ok {

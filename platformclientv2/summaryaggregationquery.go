...
func (o *Summaryaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Summaryaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Summaryaggregationquery) UnmarshalJSON(b []byte) error {
	var SummaryaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &SummaryaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := SummaryaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := SummaryaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := SummaryaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := SummaryaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := SummaryaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := SummaryaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := SummaryaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := SummaryaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := SummaryaggregationqueryMap["alternateTimeDimension"].(string); ok {
		o.AlternateTimeDimension = &AlternateTimeDimension
	}
    
	if QueryType, ok := SummaryaggregationqueryMap["queryType"].(string); ok {
		o.QueryType = &QueryType
	}
    
	if SortMetric, ok := SummaryaggregationqueryMap["sortMetric"].(map[string]interface{}); ok {
		SortMetricString, _ := json.Marshal(SortMetric)
		json.Unmarshal(SortMetricString, &o.SortMetric)
	}
	
	if Limit, ok := SummaryaggregationqueryMap["limit"].(float64); ok {
		LimitInt := int(Limit)
		o.Limit = &LimitInt
	}

...
type Workplanbidgroup struct { 
	...
	WorkPlans *[]Bidgroupworkplanresponse `json:"workPlans,omitempty"`
...
func (o *Workplanbidgroup) SetField(field string, fieldValue interface{}) {
...
func (o Workplanbidgroup) MarshalJSON() ([]byte, error) {
...
func (o *Workplanbidgroup) UnmarshalJSON(b []byte) error {
	var WorkplanbidgroupMap map[string]interface{}
	err := json.Unmarshal(b, &WorkplanbidgroupMap)
	if err != nil {
		return err
	}
	
	if Name, ok := WorkplanbidgroupMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if ManagementUnit, ok := WorkplanbidgroupMap["managementUnit"].(map[string]interface{}); ok {
		ManagementUnitString, _ := json.Marshal(ManagementUnit)
		json.Unmarshal(ManagementUnitString, &o.ManagementUnit)
	}
	
	if Agents, ok := WorkplanbidgroupMap["agents"].([]interface{}); ok {
		AgentsString, _ := json.Marshal(Agents)
		json.Unmarshal(AgentsString, &o.Agents)
	}
	
	if WorkPlans, ok := WorkplanbidgroupMap["workPlans"].([]interface{}); ok {
		WorkPlansString, _ := json.Marshal(WorkPlans)
		json.Unmarshal(WorkPlansString, &o.WorkPlans)
	}
	
	if PlanningGroups, ok := WorkplanbidgroupMap["planningGroups"].([]interface{}); ok {

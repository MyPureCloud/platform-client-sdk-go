...
type Conversationeventtopiccall struct { 
	...
	// Disposition
	Disposition *Conversationeventtopicdisposition `json:"disposition,omitempty"`
...
func (o *Conversationeventtopiccall) UnmarshalJSON(b []byte) error {
	...
	if Disposition, ok := ConversationeventtopiccallMap["disposition"].(map[string]interface{}); ok {
		DispositionString, _ := json.Marshal(Disposition)
		json.Unmarshal(DispositionString, &o.Disposition)
	}
	
	if TransferSource, ok := ConversationeventtopiccallMap["transferSource"].(string); ok {
		o.TransferSource = &TransferSource
	}
    
	if PublicIpAddress, ok := ConversationeventtopiccallMap["publicIpAddress"].(string); ok {

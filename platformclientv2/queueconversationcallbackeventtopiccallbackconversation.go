...
type Queueconversationcallbackeventtopiccallbackconversation struct { 
	...
	// Divisions
	Divisions *[]Queueconversationcallbackeventtopicconversationdivisionmembership `json:"divisions,omitempty"`
}
...
func (o *Queueconversationcallbackeventtopiccallbackconversation) UnmarshalJSON(b []byte) error {
	...
	if UtilizationLabelId, ok := QueueconversationcallbackeventtopiccallbackconversationMap["utilizationLabelId"].(string); ok {
		o.UtilizationLabelId = &UtilizationLabelId
	}
    
	if AccessAttributes, ok := QueueconversationcallbackeventtopiccallbackconversationMap["accessAttributes"].([]interface{}); ok {
		AccessAttributesString, _ := json.Marshal(AccessAttributes)
		json.Unmarshal(AccessAttributesString, &o.AccessAttributes)
	}
	
	if inactivityTimeoutString, ok := QueueconversationcallbackeventtopiccallbackconversationMap["inactivityTimeout"].(string); ok {
		InactivityTimeout, _ := time.Parse("2006-01-02T15:04:05.999999Z", inactivityTimeoutString)
		o.InactivityTimeout = &InactivityTimeout
	}
	
	if Divisions, ok := QueueconversationcallbackeventtopiccallbackconversationMap["divisions"].([]interface{}); ok {

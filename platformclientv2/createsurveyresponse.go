...
type Createsurveyresponse struct { 
	...
	CustomerSurveyUrl *string `json:"customerSurveyUrl,omitempty"`
...
func (o *Createsurveyresponse) SetField(field string, fieldValue interface{}) {
...
func (o Createsurveyresponse) MarshalJSON() ([]byte, error) {
	...
	return json.Marshal(&struct { 
		...
		CustomerSurveyUrl: o.CustomerSurveyUrl,
...
func (o *Createsurveyresponse) UnmarshalJSON(b []byte) error {
	var CreatesurveyresponseMap map[string]interface{}
	err := json.Unmarshal(b, &CreatesurveyresponseMap)
	if err != nil {
		return err
	}
	
	if Id, ok := CreatesurveyresponseMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := CreatesurveyresponseMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Conversation, ok := CreatesurveyresponseMap["conversation"].(map[string]interface{}); ok {
		ConversationString, _ := json.Marshal(Conversation)
		json.Unmarshal(ConversationString, &o.Conversation)
	}
	
	if SurveyForm, ok := CreatesurveyresponseMap["surveyForm"].(map[string]interface{}); ok {
		SurveyFormString, _ := json.Marshal(SurveyForm)
		json.Unmarshal(SurveyFormString, &o.SurveyForm)
	}
	
	if Agent, ok := CreatesurveyresponseMap["agent"].(map[string]interface{}); ok {
		AgentString, _ := json.Marshal(Agent)
		json.Unmarshal(AgentString, &o.Agent)
	}
	
	if Status, ok := CreatesurveyresponseMap["status"].(string); ok {
		o.Status = &Status
	}
    
	if Queue, ok := CreatesurveyresponseMap["queue"].(map[string]interface{}); ok {
		QueueString, _ := json.Marshal(Queue)
		json.Unmarshal(QueueString, &o.Queue)
	}
	
	if Answers, ok := CreatesurveyresponseMap["answers"].(map[string]interface{}); ok {
		AnswersString, _ := json.Marshal(Answers)
		json.Unmarshal(AnswersString, &o.Answers)
	}
	
	if completedDateString, ok := CreatesurveyresponseMap["completedDate"].(string); ok {
		CompletedDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", completedDateString)
		o.CompletedDate = &CompletedDate
	}
	
	if SurveyErrorDetails, ok := CreatesurveyresponseMap["surveyErrorDetails"].(map[string]interface{}); ok {
		SurveyErrorDetailsString, _ := json.Marshal(SurveyErrorDetails)
		json.Unmarshal(SurveyErrorDetailsString, &o.SurveyErrorDetails)
	}
	
	if AgentTeam, ok := CreatesurveyresponseMap["agentTeam"].(map[string]interface{}); ok {
		AgentTeamString, _ := json.Marshal(AgentTeam)
		json.Unmarshal(AgentTeamString, &o.AgentTeam)
	}
	
	if SurveyType, ok := CreatesurveyresponseMap["surveyType"].(string); ok {
		o.SurveyType = &SurveyType
	}
    
	if MissingRequiredAnswer, ok := CreatesurveyresponseMap["missingRequiredAnswer"].(bool); ok {
		o.MissingRequiredAnswer = &MissingRequiredAnswer
	}
    
	if Flow, ok := CreatesurveyresponseMap["flow"].(map[string]interface{}); ok {
		FlowString, _ := json.Marshal(Flow)
		json.Unmarshal(FlowString, &o.Flow)
	}
	
	if CustomerSurveyUrl, ok := CreatesurveyresponseMap["customerSurveyUrl"].(string); ok {
		o.CustomerSurveyUrl = &CustomerSurveyUrl
	}
    
	if SelfUri, ok := CreatesurveyresponseMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

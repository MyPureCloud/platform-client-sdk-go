...
func (o *Adfs) SetField(field string, fieldValue interface{}) {
...
func (o *Adfs) UnmarshalJSON(b []byte) error {
	var AdfsMap map[string]interface{}
	err := json.Unmarshal(b, &AdfsMap)
	if err != nil {
		return err
	}
	
	if Id, ok := AdfsMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := AdfsMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Disabled, ok := AdfsMap["disabled"].(bool); ok {
		o.Disabled = &Disabled
	}
    
	if IssuerURI, ok := AdfsMap["issuerURI"].(string); ok {
		o.IssuerURI = &IssuerURI
	}
    
	if SsoTargetURI, ok := AdfsMap["ssoTargetURI"].(string); ok {
		o.SsoTargetURI = &SsoTargetURI
	}
    
	if SloURI, ok := AdfsMap["sloURI"].(string); ok {
		o.SloURI = &SloURI
	}
    
	if SloBinding, ok := AdfsMap["sloBinding"].(string); ok {
		o.SloBinding = &SloBinding
	}
    
	if RelyingPartyIdentifier, ok := AdfsMap["relyingPartyIdentifier"].(string); ok {
		o.RelyingPartyIdentifier = &RelyingPartyIdentifier
	}
    
	if Certificate, ok := AdfsMap["certificate"].(string); ok {
		o.Certificate = &Certificate
	}
    
	if Certificates, ok := AdfsMap["certificates"].([]interface{}); ok {
		CertificatesString, _ := json.Marshal(Certificates)
		json.Unmarshal(CertificatesString, &o.Certificates)
	}
	
	if LogoImageData, ok := AdfsMap["logoImageData"].(string); ok {
		o.LogoImageData = &LogoImageData
	}
    
	if NameIdentifierFormat, ok := AdfsMap["nameIdentifierFormat"].(string); ok {
		o.NameIdentifierFormat = &NameIdentifierFormat
	}
    
	if SsoBinding, ok := AdfsMap["ssoBinding"].(string); ok {
		o.SsoBinding = &SsoBinding
	}
    
	if SignAuthnRequests, ok := AdfsMap["signAuthnRequests"].(bool); ok {
		o.SignAuthnRequests = &SignAuthnRequests
	}
    
	if ProviderName, ok := AdfsMap["providerName"].(string); ok {
		o.ProviderName = &ProviderName
	}
    
	if DisplayOnLogin, ok := AdfsMap["displayOnLogin"].(bool); ok {
		o.DisplayOnLogin = &DisplayOnLogin
	}
    
	if MetadataURL, ok := AdfsMap["metadataURL"].(string); ok {
		o.MetadataURL = &MetadataURL
	}
    
	if ForceAuthn, ok := AdfsMap["forceAuthn"].(bool); ok {
		o.ForceAuthn = &ForceAuthn
	}
    
	if SelfUri, ok := AdfsMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

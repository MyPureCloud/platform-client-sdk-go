...
func (o *Contestmetricscore) SetField(field string, fieldValue interface{}) {
...
func (o Contestmetricscore) MarshalJSON() ([]byte, error) {
...
func (o *Contestmetricscore) UnmarshalJSON(b []byte) error {
	var ContestmetricscoreMap map[string]interface{}
	err := json.Unmarshal(b, &ContestmetricscoreMap)
	if err != nil {
		return err
	}
	
	if Metric, ok := ContestmetricscoreMap["metric"].(map[string]interface{}); ok {
		MetricString, _ := json.Marshal(Metric)
		json.Unmarshal(MetricString, &o.Metric)
	}
	
	if Score, ok := ContestmetricscoreMap["score"].(float64); ok {
		o.Score = &Score
	}
    
	if TotalPoints, ok := ContestmetricscoreMap["totalPoints"].(float64); ok {
		o.TotalPoints = &TotalPoints
	}
    
	if PercentOfGoal, ok := ContestmetricscoreMap["percentOfGoal"].(float64); ok {

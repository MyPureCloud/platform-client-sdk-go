...
func (o *Conversationqueuemediasettings) SetField(field string, fieldValue interface{}) {
...
func (o Conversationqueuemediasettings) MarshalJSON() ([]byte, error) {
...
func (o *Conversationqueuemediasettings) UnmarshalJSON(b []byte) error {
	var ConversationqueuemediasettingsMap map[string]interface{}
	err := json.Unmarshal(b, &ConversationqueuemediasettingsMap)
	if err != nil {
		return err
	}
	
	if AlertingTimeoutSeconds, ok := ConversationqueuemediasettingsMap["alertingTimeoutSeconds"].(float64); ok {
		AlertingTimeoutSecondsInt := int(AlertingTimeoutSeconds)
		o.AlertingTimeoutSeconds = &AlertingTimeoutSecondsInt
	}
	
	if AutoAnswerAlertToneSeconds, ok := ConversationqueuemediasettingsMap["autoAnswerAlertToneSeconds"].(float64); ok {
		o.AutoAnswerAlertToneSeconds = &AutoAnswerAlertToneSeconds
	}
    
	if ManualAnswerAlertToneSeconds, ok := ConversationqueuemediasettingsMap["manualAnswerAlertToneSeconds"].(float64); ok {
		o.ManualAnswerAlertToneSeconds = &ManualAnswerAlertToneSeconds
	}
    
	if EnableAutoAnswer, ok := ConversationqueuemediasettingsMap["enableAutoAnswer"].(bool); ok {

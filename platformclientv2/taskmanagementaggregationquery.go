...
func (o *Taskmanagementaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Taskmanagementaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Taskmanagementaggregationquery) UnmarshalJSON(b []byte) error {
	var TaskmanagementaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &TaskmanagementaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := TaskmanagementaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := TaskmanagementaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := TaskmanagementaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := TaskmanagementaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := TaskmanagementaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := TaskmanagementaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := TaskmanagementaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := TaskmanagementaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := TaskmanagementaggregationqueryMap["alternateTimeDimension"].(string); ok {

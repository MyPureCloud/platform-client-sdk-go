...
func (o *Botasyncaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Botasyncaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Botasyncaggregationquery) UnmarshalJSON(b []byte) error {
	var BotasyncaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &BotasyncaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := BotasyncaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := BotasyncaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := BotasyncaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := BotasyncaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := BotasyncaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := BotasyncaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := BotasyncaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := BotasyncaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := BotasyncaggregationqueryMap["alternateTimeDimension"].(string); ok {

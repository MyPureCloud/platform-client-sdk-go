...
func (o *Evaluationresponse) UnmarshalJSON(b []byte) error {
	...
	if DisputeCount, ok := EvaluationresponseMap["disputeCount"].(float64); ok {
		DisputeCountInt := int(DisputeCount)
		o.DisputeCount = &DisputeCountInt
	}
	
	if Version, ok := EvaluationresponseMap["version"].(float64); ok {
		VersionInt := int(Version)
		o.Version = &VersionInt
	}
	
	if VersionHistory, ok := EvaluationresponseMap["versionHistory"].(map[string]interface{}); ok {
		VersionHistoryString, _ := json.Marshal(VersionHistory)
		json.Unmarshal(VersionHistoryString, &o.VersionHistory)
	}
	
	if DeclinedReview, ok := EvaluationresponseMap["declinedReview"].(bool); ok {
		o.DeclinedReview = &DeclinedReview
	}
    
	if EvaluationContextId, ok := EvaluationresponseMap["evaluationContextId"].(string); ok {
		o.EvaluationContextId = &EvaluationContextId
	}
    
	if RetractedEvaluation, ok := EvaluationresponseMap["retractedEvaluation"].(map[string]interface{}); ok {

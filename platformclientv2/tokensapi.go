...
func (a TokensApi) DeleteToken(userId string) (*APIResponse, error) {
	...
	path := a.Configuration.BasePath + "/api/v2/tokens/{userId}"
...
// GetTokensTimeout invokes GET /api/v2/tokens/timeout
//
...
func (a TokensApi) GetTokensTimeout() (*Idletokentimeout, *APIResponse, error) {
	var httpMethod = "GET"
	// create path and map variables
	path := a.Configuration.BasePath + "/api/v2/tokens/timeout"
	defaultReturn := new(Idletokentimeout)
	if true == false {
		return defaultReturn, nil, errors.New("This message brought to you by the laws of physics being broken")
	}
	...
	var successPayload *Idletokentimeout
	response, err := a.Configuration.APIClient.CallAPI(path, httpMethod, postBody, headerParams, queryParams, formParams, postFileName, fileBytes)
	if err != nil {
		// Nothing special to do here, but do avoid processing the response
	} else if err == nil && response.Error != nil {
...
// PutTokensTimeout invokes PUT /api/v2/tokens/timeout
//

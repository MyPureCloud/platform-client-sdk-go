...
type Summaryasyncaggregationquery struct { 
	...
	Filter *Summaryaggregatequeryfilter `json:"filter,omitempty"`
	...
	Views *[]Summaryaggregationview `json:"views,omitempty"`
...
func (o *Summaryasyncaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Summaryasyncaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Summaryasyncaggregationquery) UnmarshalJSON(b []byte) error {
	var SummaryasyncaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &SummaryasyncaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := SummaryasyncaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := SummaryasyncaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := SummaryasyncaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := SummaryasyncaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := SummaryasyncaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := SummaryasyncaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := SummaryasyncaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := SummaryasyncaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := SummaryasyncaggregationqueryMap["alternateTimeDimension"].(string); ok {
		o.AlternateTimeDimension = &AlternateTimeDimension
	}
    
	if QueryType, ok := SummaryasyncaggregationqueryMap["queryType"].(string); ok {
	...
	if PageSize, ok := SummaryasyncaggregationqueryMap["pageSize"].(float64); ok {

...
func (o *Addopportunitybody) SetField(field string, fieldValue interface{}) {
...
func (o Addopportunitybody) MarshalJSON() ([]byte, error) {
	...
	if len(o.SetFieldNames) > 0 {
		...
		dateTimeFields := []string{ "StartDate","EndDate","OpenDate","DeadlineDate", }
...
func (o *Addopportunitybody) UnmarshalJSON(b []byte) error {
	var AddopportunitybodyMap map[string]interface{}
	err := json.Unmarshal(b, &AddopportunitybodyMap)
	if err != nil {
		return err
	}
	
	if startDateString, ok := AddopportunitybodyMap["startDate"].(string); ok {
		StartDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", startDateString)
		o.StartDate = &StartDate
	}
	
	if endDateString, ok := AddopportunitybodyMap["endDate"].(string); ok {
		EndDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", endDateString)
		o.EndDate = &EndDate
	}
	
	if openDateString, ok := AddopportunitybodyMap["openDate"].(string); ok {
		OpenDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", openDateString)
		o.OpenDate = &OpenDate
	}
	
	if deadlineDateString, ok := AddopportunitybodyMap["deadlineDate"].(string); ok {
		DeadlineDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", deadlineDateString)
		o.DeadlineDate = &DeadlineDate
	}
	
	if Name, ok := AddopportunitybodyMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Description, ok := AddopportunitybodyMap["description"].(string); ok {
		o.Description = &Description
	}
    
	if ActivityCodeId, ok := AddopportunitybodyMap["activityCodeId"].(string); ok {
		o.ActivityCodeId = &ActivityCodeId
	}
    
	if ApprovalType, ok := AddopportunitybodyMap["approvalType"].(string); ok {
		o.ApprovalType = &ApprovalType
	}
    
	if Capacity, ok := AddopportunitybodyMap["capacity"].(float64); ok {

...
func (o *Bulkresponse) SetField(field string, fieldValue interface{}) {
...
func (o *Bulkresponse) UnmarshalJSON(b []byte) error {
	var BulkresponseMap map[string]interface{}
	err := json.Unmarshal(b, &BulkresponseMap)
	if err != nil {
		return err
	}
	
	if Results, ok := BulkresponseMap["results"].([]interface{}); ok {
		ResultsString, _ := json.Marshal(Results)
		json.Unmarshal(ResultsString, &o.Results)
	}
	
	if ErrorCount, ok := BulkresponseMap["errorCount"].(float64); ok {
		ErrorCountInt := int(ErrorCount)
		o.ErrorCount = &ErrorCountInt
	}
	
	if ErrorIndexes, ok := BulkresponseMap["errorIndexes"].([]interface{}); ok {

...
func (o *Knowledgeguestdocumentresponse) SetField(field string, fieldValue interface{}) {
...
func (o Knowledgeguestdocumentresponse) MarshalJSON() ([]byte, error) {
...
func (o *Knowledgeguestdocumentresponse) UnmarshalJSON(b []byte) error {
	var KnowledgeguestdocumentresponseMap map[string]interface{}
	err := json.Unmarshal(b, &KnowledgeguestdocumentresponseMap)
	if err != nil {
		return err
	}
	
	if Id, ok := KnowledgeguestdocumentresponseMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Title, ok := KnowledgeguestdocumentresponseMap["title"].(string); ok {
		o.Title = &Title
	}
    
	if Visible, ok := KnowledgeguestdocumentresponseMap["visible"].(bool); ok {
		o.Visible = &Visible
	}
    
	if Alternatives, ok := KnowledgeguestdocumentresponseMap["alternatives"].([]interface{}); ok {
		AlternativesString, _ := json.Marshal(Alternatives)
		json.Unmarshal(AlternativesString, &o.Alternatives)
	}
	
	if State, ok := KnowledgeguestdocumentresponseMap["state"].(string); ok {
		o.State = &State
	}
    
	if dateCreatedString, ok := KnowledgeguestdocumentresponseMap["dateCreated"].(string); ok {
		DateCreated, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCreatedString)
		o.DateCreated = &DateCreated
	}
	
	if dateModifiedString, ok := KnowledgeguestdocumentresponseMap["dateModified"].(string); ok {
		DateModified, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateModifiedString)
		o.DateModified = &DateModified
	}
	
	if dateImportedString, ok := KnowledgeguestdocumentresponseMap["dateImported"].(string); ok {
		DateImported, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateImportedString)
		o.DateImported = &DateImported
	}
	
	if LastPublishedVersionNumber, ok := KnowledgeguestdocumentresponseMap["lastPublishedVersionNumber"].(float64); ok {
		LastPublishedVersionNumberInt := int(LastPublishedVersionNumber)
		o.LastPublishedVersionNumber = &LastPublishedVersionNumberInt
	}
	
	if datePublishedString, ok := KnowledgeguestdocumentresponseMap["datePublished"].(string); ok {
		DatePublished, _ := time.Parse("2006-01-02T15:04:05.999999Z", datePublishedString)
		o.DatePublished = &DatePublished
	}
	
	if CreatedBy, ok := KnowledgeguestdocumentresponseMap["createdBy"].(map[string]interface{}); ok {
		CreatedByString, _ := json.Marshal(CreatedBy)
		json.Unmarshal(CreatedByString, &o.CreatedBy)
	}
	
	if ModifiedBy, ok := KnowledgeguestdocumentresponseMap["modifiedBy"].(map[string]interface{}); ok {
		ModifiedByString, _ := json.Marshal(ModifiedBy)
		json.Unmarshal(ModifiedByString, &o.ModifiedBy)
	}
	
	if DocumentVersion, ok := KnowledgeguestdocumentresponseMap["documentVersion"].(map[string]interface{}); ok {
		DocumentVersionString, _ := json.Marshal(DocumentVersion)
		json.Unmarshal(DocumentVersionString, &o.DocumentVersion)
	}
	
	if SessionId, ok := KnowledgeguestdocumentresponseMap["sessionId"].(string); ok {
		o.SessionId = &SessionId
	}
    
	if Category, ok := KnowledgeguestdocumentresponseMap["category"].(map[string]interface{}); ok {
		CategoryString, _ := json.Marshal(Category)
		json.Unmarshal(CategoryString, &o.Category)
	}
	
	if Variations, ok := KnowledgeguestdocumentresponseMap["variations"].([]interface{}); ok {
		VariationsString, _ := json.Marshal(Variations)
		json.Unmarshal(VariationsString, &o.Variations)
	}
	
	if SelfUri, ok := KnowledgeguestdocumentresponseMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

...
func (o *Learningslotfulldaytimeoffmarker) SetField(field string, fieldValue interface{}) {
...
func (o Learningslotfulldaytimeoffmarker) MarshalJSON() ([]byte, error) {
...
func (o *Learningslotfulldaytimeoffmarker) UnmarshalJSON(b []byte) error {
	var LearningslotfulldaytimeoffmarkerMap map[string]interface{}
	err := json.Unmarshal(b, &LearningslotfulldaytimeoffmarkerMap)
	if err != nil {
		return err
	}
	
	if businessUnitDateString, ok := LearningslotfulldaytimeoffmarkerMap["businessUnitDate"].(string); ok {
		BusinessUnitDate, _ := time.Parse("2006-01-02", businessUnitDateString)
		o.BusinessUnitDate = &BusinessUnitDate
	}
	
	if LengthMinutes, ok := LearningslotfulldaytimeoffmarkerMap["lengthMinutes"].(float64); ok {
		LengthMinutesInt := int(LengthMinutes)
		o.LengthMinutes = &LengthMinutesInt
	}
	
	if Description, ok := LearningslotfulldaytimeoffmarkerMap["description"].(string); ok {
		o.Description = &Description
	}
    
	if ActivityCodeId, ok := LearningslotfulldaytimeoffmarkerMap["activityCodeId"].(string); ok {
		o.ActivityCodeId = &ActivityCodeId
	}
    
	if Paid, ok := LearningslotfulldaytimeoffmarkerMap["paid"].(bool); ok {
		o.Paid = &Paid
	}
    
	if TimeOffRequestId, ok := LearningslotfulldaytimeoffmarkerMap["timeOffRequestId"].(string); ok {

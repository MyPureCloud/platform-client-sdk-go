...
// GetCoachingScheduleslotsJob invokes GET /api/v2/coaching/scheduleslots/jobs/{jobId}
//
// Retrieve the status of the job for the slots where a coaching appointment can be scheduled.
func (a CoachingApi) GetCoachingScheduleslotsJob(jobId string) (*Coachingscheduleslotsjobresponse, *APIResponse, error) {
	var httpMethod = "GET"
	// create path and map variables
	path := a.Configuration.BasePath + "/api/v2/coaching/scheduleslots/jobs/{jobId}"
	path = strings.Replace(path, "{jobId}", url.PathEscape(fmt.Sprintf("%v", jobId)), -1)
	defaultReturn := new(Coachingscheduleslotsjobresponse)
	if true == false {
		return defaultReturn, nil, errors.New("This message brought to you by the laws of physics being broken")
	}
	...
	if &jobId == nil {
		// false
		return defaultReturn, nil, errors.New("Missing required parameter 'jobId' when calling CoachingApi->GetCoachingScheduleslotsJob")
	}
	...
	var successPayload *Coachingscheduleslotsjobresponse
	response, err := a.Configuration.APIClient.CallAPI(path, httpMethod, postBody, headerParams, queryParams, formParams, postFileName, fileBytes, "other")
	if err != nil {
		// Nothing special to do here, but do avoid processing the response
	} else if err == nil && response.Error != nil {
		...
		if "Coachingscheduleslotsjobresponse" == "string" {
			copy(response.RawBody, &successPayload)
		} else {
			err = json.Unmarshal(response.RawBody, &successPayload)
		}
...
// PostCoachingScheduleslotsJobs invokes POST /api/v2/coaching/scheduleslots/jobs
//
// Start job to retrieve the slots where a coaching appointment can be scheduled.
func (a CoachingApi) PostCoachingScheduleslotsJobs(body Coachingscheduleslotsjobrequest) (*Coachingscheduleslotsjobresponse, *APIResponse, error) {
	var httpMethod = "POST"
	// create path and map variables
	path := a.Configuration.BasePath + "/api/v2/coaching/scheduleslots/jobs"
	defaultReturn := new(Coachingscheduleslotsjobresponse)
	if true == false {
		return defaultReturn, nil, errors.New("This message brought to you by the laws of physics being broken")
	}
	...
	if &body == nil {
		// false
		return defaultReturn, nil, errors.New("Missing required parameter 'body' when calling CoachingApi->PostCoachingScheduleslotsJobs")
	}

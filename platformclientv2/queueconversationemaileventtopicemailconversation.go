...
func (o *Queueconversationemaileventtopicemailconversation) UnmarshalJSON(b []byte) error {
	...
	if AccessAttributes, ok := QueueconversationemaileventtopicemailconversationMap["accessAttributes"].([]interface{}); ok {
		AccessAttributesString, _ := json.Marshal(AccessAttributes)
		json.Unmarshal(AccessAttributesString, &o.AccessAttributes)
	}
	
	if inactivityTimeoutString, ok := QueueconversationemaileventtopicemailconversationMap["inactivityTimeout"].(string); ok {

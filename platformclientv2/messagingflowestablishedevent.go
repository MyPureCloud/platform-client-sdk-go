...
func (o *Messagingflowestablishedevent) SetField(field string, fieldValue interface{}) {
...
func (o Messagingflowestablishedevent) MarshalJSON() ([]byte, error) {
...
func (o *Messagingflowestablishedevent) UnmarshalJSON(b []byte) error {
	var MessagingflowestablishedeventMap map[string]interface{}
	err := json.Unmarshal(b, &MessagingflowestablishedeventMap)
	if err != nil {
		return err
	}
	
	if EventId, ok := MessagingflowestablishedeventMap["eventId"].(string); ok {
		o.EventId = &EventId
	}
    
	if eventDateTimeString, ok := MessagingflowestablishedeventMap["eventDateTime"].(string); ok {
		EventDateTime, _ := time.Parse("2006-01-02T15:04:05.999999Z", eventDateTimeString)
		o.EventDateTime = &EventDateTime
	}
	
	if ConversationId, ok := MessagingflowestablishedeventMap["conversationId"].(string); ok {
		o.ConversationId = &ConversationId
	}
    
	if CommunicationId, ok := MessagingflowestablishedeventMap["communicationId"].(string); ok {
		o.CommunicationId = &CommunicationId
	}
    
	if InitialConfiguration, ok := MessagingflowestablishedeventMap["initialConfiguration"].(map[string]interface{}); ok {
	...
	if SourceConfiguration, ok := MessagingflowestablishedeventMap["sourceConfiguration"].(map[string]interface{}); ok {

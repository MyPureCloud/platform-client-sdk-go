...
type Queueconversationscreenshareeventtopicscreenshareconversation struct { 
	...
	// Divisions
	Divisions *[]Queueconversationscreenshareeventtopicconversationdivisionmembership `json:"divisions,omitempty"`
}

// SetField uses reflection to set a field on the model if the model has a property SetFieldNames, and triggers custom JSON serialization logic to only serialize properties that have been set using this function.
func (o *Queueconversationscreenshareeventtopicscreenshareconversation) SetField(field string, fieldValue interface{}) {
...
func (o *Queueconversationscreenshareeventtopicscreenshareconversation) UnmarshalJSON(b []byte) error {
	...
	if Address, ok := QueueconversationscreenshareeventtopicscreenshareconversationMap["address"].(string); ok {
		o.Address = &Address
	}
    
	if UtilizationLabelId, ok := QueueconversationscreenshareeventtopicscreenshareconversationMap["utilizationLabelId"].(string); ok {
		o.UtilizationLabelId = &UtilizationLabelId
	}
    
	if AccessAttributes, ok := QueueconversationscreenshareeventtopicscreenshareconversationMap["accessAttributes"].([]interface{}); ok {
		AccessAttributesString, _ := json.Marshal(AccessAttributes)
		json.Unmarshal(AccessAttributesString, &o.AccessAttributes)
	}
	
	if inactivityTimeoutString, ok := QueueconversationscreenshareeventtopicscreenshareconversationMap["inactivityTimeout"].(string); ok {
		InactivityTimeout, _ := time.Parse("2006-01-02T15:04:05.999999Z", inactivityTimeoutString)
		o.InactivityTimeout = &InactivityTimeout
	}
	
	if Divisions, ok := QueueconversationscreenshareeventtopicscreenshareconversationMap["divisions"].([]interface{}); ok {

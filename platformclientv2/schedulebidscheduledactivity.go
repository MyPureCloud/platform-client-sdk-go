...
func (o *Schedulebidscheduledactivity) SetField(field string, fieldValue interface{}) {
...
func (o Schedulebidscheduledactivity) MarshalJSON() ([]byte, error) {
...
func (o *Schedulebidscheduledactivity) UnmarshalJSON(b []byte) error {
	var SchedulebidscheduledactivityMap map[string]interface{}
	err := json.Unmarshal(b, &SchedulebidscheduledactivityMap)
	if err != nil {
		return err
	}
	
	if startDateString, ok := SchedulebidscheduledactivityMap["startDate"].(string); ok {
		StartDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", startDateString)
		o.StartDate = &StartDate
	}
	
	if LengthMinutes, ok := SchedulebidscheduledactivityMap["lengthMinutes"].(float64); ok {
		LengthMinutesInt := int(LengthMinutes)
		o.LengthMinutes = &LengthMinutesInt
	}
	
	if Description, ok := SchedulebidscheduledactivityMap["description"].(string); ok {
		o.Description = &Description
	}
    
	if ActivityCategory, ok := SchedulebidscheduledactivityMap["activityCategory"].(string); ok {
		o.ActivityCategory = &ActivityCategory
	}
    
	if ActivityCodeId, ok := SchedulebidscheduledactivityMap["activityCodeId"].(string); ok {
		o.ActivityCodeId = &ActivityCodeId
	}
    
	if Paid, ok := SchedulebidscheduledactivityMap["paid"].(bool); ok {
		o.Paid = &Paid
	}

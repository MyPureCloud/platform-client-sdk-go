...
func (o *Dispositionamdtimeout) SetField(field string, fieldValue interface{}) {
...
func (o Dispositionamdtimeout) MarshalJSON() ([]byte, error) {
...
func (o *Dispositionamdtimeout) UnmarshalJSON(b []byte) error {
	var DispositionamdtimeoutMap map[string]interface{}
	err := json.Unmarshal(b, &DispositionamdtimeoutMap)
	if err != nil {
		return err
	}
	
	if TimeoutMs, ok := DispositionamdtimeoutMap["timeoutMs"].(float64); ok {
		TimeoutMsInt := int(TimeoutMs)
		o.TimeoutMs = &TimeoutMsInt
	}
	
	if TimerStartEvent, ok := DispositionamdtimeoutMap["timerStartEvent"].(string); ok {
		o.TimerStartEvent = &TimerStartEvent
	}
    
	if timerStartTimeString, ok := DispositionamdtimeoutMap["timerStartTime"].(string); ok {
		TimerStartTime, _ := time.Parse("2006-01-02T15:04:05.999999Z", timerStartTimeString)
		o.TimerStartTime = &TimerStartTime
	}
	
	if timerEndTimeString, ok := DispositionamdtimeoutMap["timerEndTime"].(string); ok {

...
type Workitemsusereventsnotificationworkitem struct { 
	...
	// PropertyChanges
	PropertyChanges *[]Workitemsusereventsnotificationpropertychange `json:"propertyChanges,omitempty"`
	...
	// ScoredAgents
	ScoredAgents *[]Workitemsusereventsnotificationscoredagent `json:"scoredAgents,omitempty"`
...
func (o *Workitemsusereventsnotificationworkitem) UnmarshalJSON(b []byte) error {
	...
	if PropertyChanges, ok := WorkitemsusereventsnotificationworkitemMap["propertyChanges"].([]interface{}); ok {
	...
	if WorkbinName, ok := WorkitemsusereventsnotificationworkitemMap["workbinName"].(string); ok {
		o.WorkbinName = &WorkbinName
	}
    
	if TypeName, ok := WorkitemsusereventsnotificationworkitemMap["typeName"].(string); ok {
		o.TypeName = &TypeName
	}
    
	if PreferredAgentIds, ok := WorkitemsusereventsnotificationworkitemMap["preferredAgentIds"].([]interface{}); ok {
		PreferredAgentIdsString, _ := json.Marshal(PreferredAgentIds)
		json.Unmarshal(PreferredAgentIdsString, &o.PreferredAgentIds)
	}
	
	if DivisionId, ok := WorkitemsusereventsnotificationworkitemMap["divisionId"].(string); ok {
		o.DivisionId = &DivisionId
	}
    
	if ScoredAgents, ok := WorkitemsusereventsnotificationworkitemMap["scoredAgents"].([]interface{}); ok {
		ScoredAgentsString, _ := json.Marshal(ScoredAgents)
		json.Unmarshal(ScoredAgentsString, &o.ScoredAgents)
	}
	
	if UtilizationLabelId, ok := WorkitemsusereventsnotificationworkitemMap["utilizationLabelId"].(string); ok {
		o.UtilizationLabelId = &UtilizationLabelId
	}
    
	if CaseId, ok := WorkitemsusereventsnotificationworkitemMap["caseId"].(string); ok {
		o.CaseId = &CaseId
	}

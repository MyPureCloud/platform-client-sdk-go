...
func (o *Facebookdataingestionruleresponse) UnmarshalJSON(b []byte) error {
	...
	if IngestionRuleInfo, ok := FacebookdataingestionruleresponseMap["ingestionRuleInfo"].(map[string]interface{}); ok {
		IngestionRuleInfoString, _ := json.Marshal(IngestionRuleInfo)
		json.Unmarshal(IngestionRuleInfoString, &o.IngestionRuleInfo)
	}
	
	if Countries, ok := FacebookdataingestionruleresponseMap["countries"].([]interface{}); ok {

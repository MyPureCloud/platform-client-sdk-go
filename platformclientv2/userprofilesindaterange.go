...
type Userprofilesindaterange struct { 
	...
	Profiles *[]Profilewithdaterange `json:"profiles,omitempty"`
}

// SetField uses reflection to set a field on the model if the model has a property SetFieldNames, and triggers custom JSON serialization logic to only serialize properties that have been set using this function.
func (o *Userprofilesindaterange) SetField(field string, fieldValue interface{}) {
...
func (o Userprofilesindaterange) MarshalJSON() ([]byte, error) {
...
func (o *Userprofilesindaterange) UnmarshalJSON(b []byte) error {
	var UserprofilesindaterangeMap map[string]interface{}
	err := json.Unmarshal(b, &UserprofilesindaterangeMap)
	if err != nil {
		return err
	}
	
	if User, ok := UserprofilesindaterangeMap["user"].(map[string]interface{}); ok {
		UserString, _ := json.Marshal(User)
		json.Unmarshal(UserString, &o.User)
	}
	
	if dateStartWorkdayString, ok := UserprofilesindaterangeMap["dateStartWorkday"].(string); ok {
		DateStartWorkday, _ := time.Parse("2006-01-02", dateStartWorkdayString)
		o.DateStartWorkday = &DateStartWorkday
	}
	
	if dateEndWorkdayString, ok := UserprofilesindaterangeMap["dateEndWorkday"].(string); ok {
		DateEndWorkday, _ := time.Parse("2006-01-02", dateEndWorkdayString)
		o.DateEndWorkday = &DateEndWorkday
	}
	
	if Profiles, ok := UserprofilesindaterangeMap["profiles"].([]interface{}); ok {
		ProfilesString, _ := json.Marshal(Profiles)
		json.Unmarshal(ProfilesString, &o.Profiles)
	}

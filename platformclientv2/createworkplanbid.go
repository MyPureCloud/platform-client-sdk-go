...
func (o *Createworkplanbid) SetField(field string, fieldValue interface{}) {
...
func (o Createworkplanbid) MarshalJSON() ([]byte, error) {
...
func (o *Createworkplanbid) UnmarshalJSON(b []byte) error {
	var CreateworkplanbidMap map[string]interface{}
	err := json.Unmarshal(b, &CreateworkplanbidMap)
	if err != nil {
		return err
	}
	
	if Name, ok := CreateworkplanbidMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Forecast, ok := CreateworkplanbidMap["forecast"].(map[string]interface{}); ok {
		ForecastString, _ := json.Marshal(Forecast)
		json.Unmarshal(ForecastString, &o.Forecast)
	}
	
	if bidWindowStartDateString, ok := CreateworkplanbidMap["bidWindowStartDate"].(string); ok {
		BidWindowStartDate, _ := time.Parse("2006-01-02", bidWindowStartDateString)
		o.BidWindowStartDate = &BidWindowStartDate
	}
	
	if bidWindowEndDateString, ok := CreateworkplanbidMap["bidWindowEndDate"].(string); ok {
		BidWindowEndDate, _ := time.Parse("2006-01-02", bidWindowEndDateString)
		o.BidWindowEndDate = &BidWindowEndDate
	}
	
	if effectiveDateString, ok := CreateworkplanbidMap["effectiveDate"].(string); ok {
		EffectiveDate, _ := time.Parse("2006-01-02", effectiveDateString)
		o.EffectiveDate = &EffectiveDate
	}
	
	if AgentRankingType, ok := CreateworkplanbidMap["agentRankingType"].(string); ok {
		o.AgentRankingType = &AgentRankingType
	}
    
	if RankingTiebreakerType, ok := CreateworkplanbidMap["rankingTiebreakerType"].(string); ok {
		o.RankingTiebreakerType = &RankingTiebreakerType
	}
    
	if EndOverridesAndRotations, ok := CreateworkplanbidMap["endOverridesAndRotations"].(bool); ok {
		o.EndOverridesAndRotations = &EndOverridesAndRotations
	}
    
	if WorkPlanFieldsVisibleToAgents, ok := CreateworkplanbidMap["workPlanFieldsVisibleToAgents"].([]interface{}); ok {

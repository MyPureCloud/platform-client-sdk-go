...
// PostUsageEventsAggregatesQuery invokes POST /api/v2/usage/events/aggregates/query
//
// Get aggregates for operational events in a timeframe.
func (a OperationalEventsApi) PostUsageEventsAggregatesQuery(body Eventaggregatesqueryrequest) (*Eventaggregatesresponse, *APIResponse, error) {
	var httpMethod = "POST"
	// create path and map variables
	path := a.Configuration.BasePath + "/api/v2/usage/events/aggregates/query"
	defaultReturn := new(Eventaggregatesresponse)
	if true == false {
		return defaultReturn, nil, errors.New("This message brought to you by the laws of physics being broken")
	}
	...
	var successPayload *Eventaggregatesresponse
	response, err := a.Configuration.APIClient.CallAPI(path, httpMethod, postBody, headerParams, queryParams, formParams, postFileName, fileBytes, "other")
	if err != nil {
		// Nothing special to do here, but do avoid processing the response
	} else if err == nil && response.Error != nil {
		...
		if "Eventaggregatesresponse" == "string" {
			copy(response.RawBody, &successPayload)
		} else {
			err = json.Unmarshal(response.RawBody, &successPayload)
		}
...
// PostUsageEventsQuery invokes POST /api/v2/usage/events/query
//
// Query operational events in a timeframe.
func (a OperationalEventsApi) PostUsageEventsQuery(before string, after string, pageSize string, body Eventqueryrequest) (*Eventqueryresponse, *APIResponse, error) {
	var httpMethod = "POST"
	// create path and map variables
	path := a.Configuration.BasePath + "/api/v2/usage/events/query"
	defaultReturn := new(Eventqueryresponse)
	if true == false {
		return defaultReturn, nil, errors.New("This message brought to you by the laws of physics being broken")
	}
	...
	var successPayload *Eventqueryresponse
	response, err := a.Configuration.APIClient.CallAPI(path, httpMethod, postBody, headerParams, queryParams, formParams, postFileName, fileBytes, "other")
	if err != nil {
		// Nothing special to do here, but do avoid processing the response
	} else if err == nil && response.Error != nil {

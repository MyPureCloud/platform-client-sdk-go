...
func (o *Queueconversationvideoeventtopicmessage) UnmarshalJSON(b []byte) error {
	...
	if EngagementSource, ok := QueueconversationvideoeventtopicmessageMap["engagementSource"].(string); ok {
	...
	if QueueMediaSettings, ok := QueueconversationvideoeventtopicmessageMap["queueMediaSettings"].(map[string]interface{}); ok {
		QueueMediaSettingsString, _ := json.Marshal(QueueMediaSettings)
		json.Unmarshal(QueueMediaSettingsString, &o.QueueMediaSettings)
	}
	
	if resumeTimeString, ok := QueueconversationvideoeventtopicmessageMap["resumeTime"].(string); ok {
		ResumeTime, _ := time.Parse("2006-01-02T15:04:05.999999Z", resumeTimeString)
		o.ResumeTime = &ResumeTime
	}
	
	if parkTimeString, ok := QueueconversationvideoeventtopicmessageMap["parkTime"].(string); ok {

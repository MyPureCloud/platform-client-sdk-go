...
type Insightsagents struct { 
	...
	Entities *[]Insightsagentitem `json:"entities,omitempty"`
}

// SetField uses reflection to set a field on the model if the model has a property SetFieldNames, and triggers custom JSON serialization logic to only serialize properties that have been set using this function.
func (o *Insightsagents) SetField(field string, fieldValue interface{}) {
...
func (o Insightsagents) MarshalJSON() ([]byte, error) {
...
func (o *Insightsagents) UnmarshalJSON(b []byte) error {
	var InsightsagentsMap map[string]interface{}
	err := json.Unmarshal(b, &InsightsagentsMap)
	if err != nil {
		return err
	}
	
	if PerformanceProfile, ok := InsightsagentsMap["performanceProfile"].(map[string]interface{}); ok {
		PerformanceProfileString, _ := json.Marshal(PerformanceProfile)
		json.Unmarshal(PerformanceProfileString, &o.PerformanceProfile)
	}
	
	if Division, ok := InsightsagentsMap["division"].(map[string]interface{}); ok {
		DivisionString, _ := json.Marshal(Division)
		json.Unmarshal(DivisionString, &o.Division)
	}
	
	if Granularity, ok := InsightsagentsMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if dateStartWorkdayString, ok := InsightsagentsMap["dateStartWorkday"].(string); ok {
		DateStartWorkday, _ := time.Parse("2006-01-02", dateStartWorkdayString)
		o.DateStartWorkday = &DateStartWorkday
	}
	
	if dateEndWorkdayString, ok := InsightsagentsMap["dateEndWorkday"].(string); ok {
		DateEndWorkday, _ := time.Parse("2006-01-02", dateEndWorkdayString)
		o.DateEndWorkday = &DateEndWorkday
	}
	
	if Entities, ok := InsightsagentsMap["entities"].([]interface{}); ok {

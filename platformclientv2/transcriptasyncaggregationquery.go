...
func (o *Transcriptasyncaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Transcriptasyncaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Transcriptasyncaggregationquery) UnmarshalJSON(b []byte) error {
	var TranscriptasyncaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &TranscriptasyncaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := TranscriptasyncaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := TranscriptasyncaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := TranscriptasyncaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := TranscriptasyncaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := TranscriptasyncaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := TranscriptasyncaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := TranscriptasyncaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := TranscriptasyncaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := TranscriptasyncaggregationqueryMap["alternateTimeDimension"].(string); ok {

...
type Ratelimitaggregationquery struct { 
	...
	Filter *Ratelimitaggregatequeryfilter `json:"filter,omitempty"`
	...
	Views *[]Ratelimitaggregationview `json:"views,omitempty"`
...
func (o *Ratelimitaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Ratelimitaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Ratelimitaggregationquery) UnmarshalJSON(b []byte) error {
	var RatelimitaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &RatelimitaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := RatelimitaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := RatelimitaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := RatelimitaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := RatelimitaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := RatelimitaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := RatelimitaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := RatelimitaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := RatelimitaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := RatelimitaggregationqueryMap["alternateTimeDimension"].(string); ok {

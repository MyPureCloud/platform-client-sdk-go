...
func (o *Socialmediaasyncaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Socialmediaasyncaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Socialmediaasyncaggregationquery) UnmarshalJSON(b []byte) error {
	var SocialmediaasyncaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &SocialmediaasyncaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := SocialmediaasyncaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := SocialmediaasyncaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := SocialmediaasyncaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := SocialmediaasyncaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := SocialmediaasyncaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := SocialmediaasyncaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if PageSize, ok := SocialmediaasyncaggregationqueryMap["pageSize"].(float64); ok {

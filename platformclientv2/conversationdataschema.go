...
type Conversationdataschema struct { 
	...
	JsonSchema *Conversationjsonschemadocument `json:"jsonSchema,omitempty"`
...
func (o *Conversationdataschema) SetField(field string, fieldValue interface{}) {
...
func (o Conversationdataschema) MarshalJSON() ([]byte, error) {
...
func (o *Conversationdataschema) UnmarshalJSON(b []byte) error {
	var ConversationdataschemaMap map[string]interface{}
	err := json.Unmarshal(b, &ConversationdataschemaMap)
	if err != nil {
		return err
	}
	
	if Id, ok := ConversationdataschemaMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Version, ok := ConversationdataschemaMap["version"].(float64); ok {
		VersionInt := int(Version)
		o.Version = &VersionInt
	}
	
	if Enabled, ok := ConversationdataschemaMap["enabled"].(bool); ok {
		o.Enabled = &Enabled
	}
    
	if dateCreatedString, ok := ConversationdataschemaMap["dateCreated"].(string); ok {
		DateCreated, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCreatedString)
		o.DateCreated = &DateCreated
	}
	
	if CreatedBy, ok := ConversationdataschemaMap["createdBy"].(map[string]interface{}); ok {
		CreatedByString, _ := json.Marshal(CreatedBy)
		json.Unmarshal(CreatedByString, &o.CreatedBy)
	}
	
	if JsonSchema, ok := ConversationdataschemaMap["jsonSchema"].(map[string]interface{}); ok {
		JsonSchemaString, _ := json.Marshal(JsonSchema)
		json.Unmarshal(JsonSchemaString, &o.JsonSchema)
	}
	
	if SelfUri, ok := ConversationdataschemaMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

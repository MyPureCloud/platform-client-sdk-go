...
type Enrichfieldrule struct { 
	...
	ArrayAction *string `json:"arrayAction,omitempty"`
}

// SetField uses reflection to set a field on the model if the model has a property SetFieldNames, and triggers custom JSON serialization logic to only serialize properties that have been set using this function.
func (o *Enrichfieldrule) SetField(field string, fieldValue interface{}) {
...
func (o Enrichfieldrule) MarshalJSON() ([]byte, error) {
	...
	return json.Marshal(&struct { 
		...
		ArrayAction: o.ArrayAction,
		Alias:    (Alias)(o),
	})
}

func (o *Enrichfieldrule) UnmarshalJSON(b []byte) error {
	var EnrichfieldruleMap map[string]interface{}
	err := json.Unmarshal(b, &EnrichfieldruleMap)
	if err != nil {
		return err
	}
	
	if Field, ok := EnrichfieldruleMap["field"].(string); ok {
		o.Field = &Field
	}
    
	if Action, ok := EnrichfieldruleMap["action"].(string); ok {
		o.Action = &Action
	}
    
	if ArrayAction, ok := EnrichfieldruleMap["arrayAction"].(string); ok {
		o.ArrayAction = &ArrayAction
	}

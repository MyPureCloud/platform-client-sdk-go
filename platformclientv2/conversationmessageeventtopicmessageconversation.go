...
type Conversationmessageeventtopicmessageconversation struct { 
	...
	// Divisions
	Divisions *[]Conversationmessageeventtopicconversationdivisionmembership `json:"divisions,omitempty"`
}

// SetField uses reflection to set a field on the model if the model has a property SetFieldNames, and triggers custom JSON serialization logic to only serialize properties that have been set using this function.
func (o *Conversationmessageeventtopicmessageconversation) SetField(field string, fieldValue interface{}) {
...
func (o *Conversationmessageeventtopicmessageconversation) UnmarshalJSON(b []byte) error {
	...
	if Address, ok := ConversationmessageeventtopicmessageconversationMap["address"].(string); ok {
		o.Address = &Address
	}
    
	if UtilizationLabelId, ok := ConversationmessageeventtopicmessageconversationMap["utilizationLabelId"].(string); ok {
		o.UtilizationLabelId = &UtilizationLabelId
	}
    
	if AccessAttributes, ok := ConversationmessageeventtopicmessageconversationMap["accessAttributes"].([]interface{}); ok {
		AccessAttributesString, _ := json.Marshal(AccessAttributes)
		json.Unmarshal(AccessAttributesString, &o.AccessAttributes)
	}
	
	if inactivityTimeoutString, ok := ConversationmessageeventtopicmessageconversationMap["inactivityTimeout"].(string); ok {
		InactivityTimeout, _ := time.Parse("2006-01-02T15:04:05.999999Z", inactivityTimeoutString)
		o.InactivityTimeout = &InactivityTimeout
	}
	
	if Divisions, ok := ConversationmessageeventtopicmessageconversationMap["divisions"].([]interface{}); ok {

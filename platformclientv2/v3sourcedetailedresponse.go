...
func (o *V3sourcedetailedresponse) SetField(field string, fieldValue interface{}) {
...
func (o V3sourcedetailedresponse) MarshalJSON() ([]byte, error) {
...
func (o *V3sourcedetailedresponse) UnmarshalJSON(b []byte) error {
	var V3sourcedetailedresponseMap map[string]interface{}
	err := json.Unmarshal(b, &V3sourcedetailedresponseMap)
	if err != nil {
		return err
	}
	
	if Id, ok := V3sourcedetailedresponseMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := V3sourcedetailedresponseMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if ConnectionId, ok := V3sourcedetailedresponseMap["connectionId"].(string); ok {
		o.ConnectionId = &ConnectionId
	}
    
	if VarType, ok := V3sourcedetailedresponseMap["type"].(string); ok {
		o.VarType = &VarType
	}
    
	if TriggerType, ok := V3sourcedetailedresponseMap["triggerType"].(string); ok {
		o.TriggerType = &TriggerType
	}
    
	if Status, ok := V3sourcedetailedresponseMap["status"].(string); ok {
		o.Status = &Status
	}
    
	if CreatedBy, ok := V3sourcedetailedresponseMap["createdBy"].(map[string]interface{}); ok {
		CreatedByString, _ := json.Marshal(CreatedBy)
		json.Unmarshal(CreatedByString, &o.CreatedBy)
	}
	
	if ModifiedBy, ok := V3sourcedetailedresponseMap["modifiedBy"].(map[string]interface{}); ok {
		ModifiedByString, _ := json.Marshal(ModifiedBy)
		json.Unmarshal(ModifiedByString, &o.ModifiedBy)
	}
	
	if dateCreatedString, ok := V3sourcedetailedresponseMap["dateCreated"].(string); ok {
		DateCreated, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCreatedString)
		o.DateCreated = &DateCreated
	}
	
	if dateModifiedString, ok := V3sourcedetailedresponseMap["dateModified"].(string); ok {
		DateModified, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateModifiedString)
		o.DateModified = &DateModified
	}
	
	if LastSync, ok := V3sourcedetailedresponseMap["lastSync"].(map[string]interface{}); ok {
		LastSyncString, _ := json.Marshal(LastSync)
		json.Unmarshal(LastSyncString, &o.LastSync)
	}
	
	if ScheduleSettings, ok := V3sourcedetailedresponseMap["scheduleSettings"].(map[string]interface{}); ok {
		ScheduleSettingsString, _ := json.Marshal(ScheduleSettings)
		json.Unmarshal(ScheduleSettingsString, &o.ScheduleSettings)
	}
	
	if Filters, ok := V3sourcedetailedresponseMap["filters"].(map[string]interface{}); ok {
		FiltersString, _ := json.Marshal(Filters)
		json.Unmarshal(FiltersString, &o.Filters)
	}
	
	if FilterDetails, ok := V3sourcedetailedresponseMap["filterDetails"].(map[string]interface{}); ok {
		FilterDetailsString, _ := json.Marshal(FilterDetails)
		json.Unmarshal(FilterDetailsString, &o.FilterDetails)
	}
	
	if SelfUri, ok := V3sourcedetailedresponseMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

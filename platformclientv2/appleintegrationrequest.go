...
func (o *Appleintegrationrequest) SetField(field string, fieldValue interface{}) {
...
func (o Appleintegrationrequest) MarshalJSON() ([]byte, error) {
...
func (o *Appleintegrationrequest) UnmarshalJSON(b []byte) error {
	var AppleintegrationrequestMap map[string]interface{}
	err := json.Unmarshal(b, &AppleintegrationrequestMap)
	if err != nil {
		return err
	}
	
	if Id, ok := AppleintegrationrequestMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := AppleintegrationrequestMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if SupportedContent, ok := AppleintegrationrequestMap["supportedContent"].(map[string]interface{}); ok {
		SupportedContentString, _ := json.Marshal(SupportedContent)
		json.Unmarshal(SupportedContentString, &o.SupportedContent)
	}
	
	if MessagingSetting, ok := AppleintegrationrequestMap["messagingSetting"].(map[string]interface{}); ok {
		MessagingSettingString, _ := json.Marshal(MessagingSetting)
		json.Unmarshal(MessagingSettingString, &o.MessagingSetting)
	}
	
	if MessagesForBusinessId, ok := AppleintegrationrequestMap["messagesForBusinessId"].(string); ok {
		o.MessagesForBusinessId = &MessagesForBusinessId
	}
    
	if BusinessName, ok := AppleintegrationrequestMap["businessName"].(string); ok {
		o.BusinessName = &BusinessName
	}
    
	if LogoUrl, ok := AppleintegrationrequestMap["logoUrl"].(string); ok {
		o.LogoUrl = &LogoUrl
	}
    
	if AppleIMessageApp, ok := AppleintegrationrequestMap["appleIMessageApp"].(map[string]interface{}); ok {
		AppleIMessageAppString, _ := json.Marshal(AppleIMessageApp)
		json.Unmarshal(AppleIMessageAppString, &o.AppleIMessageApp)
	}
	
	if AppleAuthentication, ok := AppleintegrationrequestMap["appleAuthentication"].(map[string]interface{}); ok {
	...
	if ApplePay, ok := AppleintegrationrequestMap["applePay"].(map[string]interface{}); ok {
		ApplePayString, _ := json.Marshal(ApplePay)
		json.Unmarshal(ApplePayString, &o.ApplePay)
	}
	
	if SelfUri, ok := AppleintegrationrequestMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

...
func (o *Buagentscheduleshiftrequest) SetField(field string, fieldValue interface{}) {
...
func (o Buagentscheduleshiftrequest) MarshalJSON() ([]byte, error) {
...
func (o *Buagentscheduleshiftrequest) UnmarshalJSON(b []byte) error {
	var BuagentscheduleshiftrequestMap map[string]interface{}
	err := json.Unmarshal(b, &BuagentscheduleshiftrequestMap)
	if err != nil {
		return err
	}
	
	if Id, ok := BuagentscheduleshiftrequestMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if startDateString, ok := BuagentscheduleshiftrequestMap["startDate"].(string); ok {
		StartDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", startDateString)
		o.StartDate = &StartDate
	}
	
	if LengthMinutes, ok := BuagentscheduleshiftrequestMap["lengthMinutes"].(float64); ok {
		LengthMinutesInt := int(LengthMinutes)
		o.LengthMinutes = &LengthMinutesInt
	}
	
	if Activities, ok := BuagentscheduleshiftrequestMap["activities"].([]interface{}); ok {
		ActivitiesString, _ := json.Marshal(Activities)
		json.Unmarshal(ActivitiesString, &o.Activities)
	}
	
	if ManuallyEdited, ok := BuagentscheduleshiftrequestMap["manuallyEdited"].(bool); ok {

...
type Capacityplanrequest struct { 
	...
	StaffingGroupAllocations *[]Createstaffinggroupallocation `json:"staffingGroupAllocations,omitempty"`
}

// SetField uses reflection to set a field on the model if the model has a property SetFieldNames, and triggers custom JSON serialization logic to only serialize properties that have been set using this function.
func (o *Capacityplanrequest) SetField(field string, fieldValue interface{}) {
...
func (o Capacityplanrequest) MarshalJSON() ([]byte, error) {
...
func (o *Capacityplanrequest) UnmarshalJSON(b []byte) error {
	var CapacityplanrequestMap map[string]interface{}
	err := json.Unmarshal(b, &CapacityplanrequestMap)
	if err != nil {
		return err
	}
	
	if Name, ok := CapacityplanrequestMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Description, ok := CapacityplanrequestMap["description"].(string); ok {
		o.Description = &Description
	}
    
	if startBusinessUnitDateString, ok := CapacityplanrequestMap["startBusinessUnitDate"].(string); ok {
		StartBusinessUnitDate, _ := time.Parse("2006-01-02", startBusinessUnitDateString)
		o.StartBusinessUnitDate = &StartBusinessUnitDate
	}
	
	if endBusinessUnitDateString, ok := CapacityplanrequestMap["endBusinessUnitDate"].(string); ok {
		EndBusinessUnitDate, _ := time.Parse("2006-01-02", endBusinessUnitDateString)
		o.EndBusinessUnitDate = &EndBusinessUnitDate
	}
	
	if Forecast, ok := CapacityplanrequestMap["forecast"].(map[string]interface{}); ok {
		ForecastString, _ := json.Marshal(Forecast)
		json.Unmarshal(ForecastString, &o.Forecast)
	}
	
	if FullTimeEquivalentWeeklyHours, ok := CapacityplanrequestMap["fullTimeEquivalentWeeklyHours"].(float64); ok {
		o.FullTimeEquivalentWeeklyHours = &FullTimeEquivalentWeeklyHours
	}
    
	if StaffingGroupAllocations, ok := CapacityplanrequestMap["staffingGroupAllocations"].([]interface{}); ok {

...
func (o *Range) SetField(field string, fieldValue interface{}) {
...
func (o *Range) UnmarshalJSON(b []byte) error {
	var RangeMap map[string]interface{}
	err := json.Unmarshal(b, &RangeMap)
	if err != nil {
		return err
	}
	
	if VarType, ok := RangeMap["type"].(string); ok {
		o.VarType = &VarType
	}
    
	if End, ok := RangeMap["end"].(string); ok {
		o.End = &End
	}
    
	if NumberOfOccurrences, ok := RangeMap["numberOfOccurrences"].(float64); ok {

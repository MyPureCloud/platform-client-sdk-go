...
func (o *Emailconversation) UnmarshalJSON(b []byte) error {
	...
	if RecentTransfers, ok := EmailconversationMap["recentTransfers"].([]interface{}); ok {
	...
	if inactivityTimeoutString, ok := EmailconversationMap["inactivityTimeout"].(string); ok {
		InactivityTimeout, _ := time.Parse("2006-01-02T15:04:05.999999Z", inactivityTimeoutString)
		o.InactivityTimeout = &InactivityTimeout
	}
	
	if Divisions, ok := EmailconversationMap["divisions"].([]interface{}); ok {

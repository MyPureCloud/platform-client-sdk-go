...
func (o *Assignedlearningmodule) UnmarshalJSON(b []byte) error {
	...
	if ExcludedFromCatalog, ok := AssignedlearningmoduleMap["excludedFromCatalog"].(bool); ok {
	...
	if EnforceContentOrder, ok := AssignedlearningmoduleMap["enforceContentOrder"].(bool); ok {
		o.EnforceContentOrder = &EnforceContentOrder
	}
    
	if ReviewAssessmentResults, ok := AssignedlearningmoduleMap["reviewAssessmentResults"].(map[string]interface{}); ok {
	...
	if AutoAssign, ok := AssignedlearningmoduleMap["autoAssign"].(map[string]interface{}); ok {
	...
	if LengthInMinutes, ok := AssignedlearningmoduleMap["lengthInMinutes"].(float64); ok {

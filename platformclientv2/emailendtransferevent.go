...
func (o *Emailendtransferevent) SetField(field string, fieldValue interface{}) {
...
func (o Emailendtransferevent) MarshalJSON() ([]byte, error) {
...
func (o *Emailendtransferevent) UnmarshalJSON(b []byte) error {
	var EmailendtransfereventMap map[string]interface{}
	err := json.Unmarshal(b, &EmailendtransfereventMap)
	if err != nil {
		return err
	}
	
	if EventId, ok := EmailendtransfereventMap["eventId"].(string); ok {
		o.EventId = &EventId
	}
    
	if eventDateTimeString, ok := EmailendtransfereventMap["eventDateTime"].(string); ok {
		EventDateTime, _ := time.Parse("2006-01-02T15:04:05.999999Z", eventDateTimeString)
		o.EventDateTime = &EventDateTime
	}
	
	if ConversationId, ok := EmailendtransfereventMap["conversationId"].(string); ok {
		o.ConversationId = &ConversationId
	}
    
	if CommandId, ok := EmailendtransfereventMap["commandId"].(string); ok {
		o.CommandId = &CommandId
	}
    
	if FinalState, ok := EmailendtransfereventMap["finalState"].(string); ok {
		o.FinalState = &FinalState
	}
    
	if ObjectCommunicationId, ok := EmailendtransfereventMap["objectCommunicationId"].(string); ok {

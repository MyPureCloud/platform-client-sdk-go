...
func (o *Createschedulebid) SetField(field string, fieldValue interface{}) {
...
func (o Createschedulebid) MarshalJSON() ([]byte, error) {
...
func (o *Createschedulebid) UnmarshalJSON(b []byte) error {
	var CreateschedulebidMap map[string]interface{}
	err := json.Unmarshal(b, &CreateschedulebidMap)
	if err != nil {
		return err
	}
	
	if Name, ok := CreateschedulebidMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Forecast, ok := CreateschedulebidMap["forecast"].(map[string]interface{}); ok {
		ForecastString, _ := json.Marshal(Forecast)
		json.Unmarshal(ForecastString, &o.Forecast)
	}
	
	if bidWindowStartDateString, ok := CreateschedulebidMap["bidWindowStartDate"].(string); ok {
		BidWindowStartDate, _ := time.Parse("2006-01-02", bidWindowStartDateString)
		o.BidWindowStartDate = &BidWindowStartDate
	}
	
	if bidWindowEndDateString, ok := CreateschedulebidMap["bidWindowEndDate"].(string); ok {
		BidWindowEndDate, _ := time.Parse("2006-01-02", bidWindowEndDateString)
		o.BidWindowEndDate = &BidWindowEndDate
	}
	
	if effectiveDateString, ok := CreateschedulebidMap["effectiveDate"].(string); ok {
		EffectiveDate, _ := time.Parse("2006-01-02", effectiveDateString)
		o.EffectiveDate = &EffectiveDate
	}
	
	if WeeksToSchedule, ok := CreateschedulebidMap["weeksToSchedule"].(float64); ok {
		WeeksToScheduleInt := int(WeeksToSchedule)
		o.WeeksToSchedule = &WeeksToScheduleInt
	}
	
	if EndOverridesAndRotations, ok := CreateschedulebidMap["endOverridesAndRotations"].(bool); ok {
		o.EndOverridesAndRotations = &EndOverridesAndRotations
	}
    
	if AgentRankingType, ok := CreateschedulebidMap["agentRankingType"].(string); ok {
		o.AgentRankingType = &AgentRankingType
	}
    
	if RankingTiebreakerType, ok := CreateschedulebidMap["rankingTiebreakerType"].(string); ok {

...
func (o *Outlier) SetField(field string, fieldValue interface{}) {
...
func (o *Outlier) UnmarshalJSON(b []byte) error {
	var OutlierMap map[string]interface{}
	err := json.Unmarshal(b, &OutlierMap)
	if err != nil {
		return err
	}
	
	if timestampString, ok := OutlierMap["timestamp"].(string); ok {
		Timestamp, _ := time.Parse("2006-01-02T15:04:05.999999Z", timestampString)
		o.Timestamp = &Timestamp
	}
	
	if Anomaly, ok := OutlierMap["anomaly"].(string); ok {
		o.Anomaly = &Anomaly
	}
    
	if NormalizedValue, ok := OutlierMap["normalizedValue"].(float64); ok {

...
func (o *Evaluationsearchaggregationdto) SetField(field string, fieldValue interface{}) {
...
func (o Evaluationsearchaggregationdto) MarshalJSON() ([]byte, error) {
...
func (o *Evaluationsearchaggregationdto) UnmarshalJSON(b []byte) error {
	var EvaluationsearchaggregationdtoMap map[string]interface{}
	err := json.Unmarshal(b, &EvaluationsearchaggregationdtoMap)
	if err != nil {
		return err
	}
	
	if Name, ok := EvaluationsearchaggregationdtoMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Field, ok := EvaluationsearchaggregationdtoMap["field"].(string); ok {
		o.Field = &Field
	}
    
	if VarType, ok := EvaluationsearchaggregationdtoMap["type"].(string); ok {
		o.VarType = &VarType
	}
    
	if Size, ok := EvaluationsearchaggregationdtoMap["size"].(float64); ok {
		SizeInt := int(Size)
		o.Size = &SizeInt
	}
	
	if CalendarInterval, ok := EvaluationsearchaggregationdtoMap["calendarInterval"].(string); ok {
		o.CalendarInterval = &CalendarInterval
	}
    
	if Format, ok := EvaluationsearchaggregationdtoMap["format"].(string); ok {
		o.Format = &Format
	}
    
	if Ranges, ok := EvaluationsearchaggregationdtoMap["ranges"].([]interface{}); ok {
		RangesString, _ := json.Marshal(Ranges)
		json.Unmarshal(RangesString, &o.Ranges)
	}
	
	if SubAggregations, ok := EvaluationsearchaggregationdtoMap["subAggregations"].([]interface{}); ok {

...
func (o *Historicaldata) SetField(field string, fieldValue interface{}) {
...
func (o *Historicaldata) UnmarshalJSON(b []byte) error {
	var HistoricaldataMap map[string]interface{}
	err := json.Unmarshal(b, &HistoricaldataMap)
	if err != nil {
		return err
	}
	
	if Daily, ok := HistoricaldataMap["daily"].(map[string]interface{}); ok {
		DailyString, _ := json.Marshal(Daily)
		json.Unmarshal(DailyString, &o.Daily)
	}
	
	if QuarterHour, ok := HistoricaldataMap["quarterHour"].(map[string]interface{}); ok {

...
func (o *Chatmediaparticipant) SetField(field string, fieldValue interface{}) {
...
func (o *Chatmediaparticipant) UnmarshalJSON(b []byte) error {
	...
	if startAlertingTimeString, ok := ChatmediaparticipantMap["startAlertingTime"].(string); ok {
	...
	if MediaRoles, ok := ChatmediaparticipantMap["mediaRoles"].([]interface{}); ok {
	...
	if parkTimeString, ok := ChatmediaparticipantMap["parkTime"].(string); ok {
		ParkTime, _ := time.Parse("2006-01-02T15:04:05.999999Z", parkTimeString)
		o.ParkTime = &ParkTime
	}
	
	if resumeTimeString, ok := ChatmediaparticipantMap["resumeTime"].(string); ok {

...
func (o *Flowactivityscoredagent) SetField(field string, fieldValue interface{}) {
...
func (o Flowactivityscoredagent) MarshalJSON() ([]byte, error) {
...
func (o *Flowactivityscoredagent) UnmarshalJSON(b []byte) error {
	var FlowactivityscoredagentMap map[string]interface{}
	err := json.Unmarshal(b, &FlowactivityscoredagentMap)
	if err != nil {
		return err
	}
	
	if AgentScore, ok := FlowactivityscoredagentMap["agentScore"].(float64); ok {
		AgentScoreInt := int(AgentScore)
		o.AgentScore = &AgentScoreInt
	}
	
	if ScoredAgentId, ok := FlowactivityscoredagentMap["scoredAgentId"].(string); ok {

...
func (o *Chatconversation) SetField(field string, fieldValue interface{}) {
...
func (o *Chatconversation) UnmarshalJSON(b []byte) error {
	...
	if RecentTransfers, ok := ChatconversationMap["recentTransfers"].([]interface{}); ok {
		RecentTransfersString, _ := json.Marshal(RecentTransfers)
		json.Unmarshal(RecentTransfersString, &o.RecentTransfers)
	}
	
	if UtilizationLabelId, ok := ChatconversationMap["utilizationLabelId"].(string); ok {
		o.UtilizationLabelId = &UtilizationLabelId
	}
    
	if inactivityTimeoutString, ok := ChatconversationMap["inactivityTimeout"].(string); ok {
		InactivityTimeout, _ := time.Parse("2006-01-02T15:04:05.999999Z", inactivityTimeoutString)
		o.InactivityTimeout = &InactivityTimeout
	}
	
	if Divisions, ok := ChatconversationMap["divisions"].([]interface{}); ok {

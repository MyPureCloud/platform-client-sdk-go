...
func (o *Evaluationformsearchresponse) SetField(field string, fieldValue interface{}) {
...
func (o Evaluationformsearchresponse) MarshalJSON() ([]byte, error) {
...
func (o *Evaluationformsearchresponse) UnmarshalJSON(b []byte) error {
	var EvaluationformsearchresponseMap map[string]interface{}
	err := json.Unmarshal(b, &EvaluationformsearchresponseMap)
	if err != nil {
		return err
	}
	
	if Id, ok := EvaluationformsearchresponseMap["id"].(string); ok {
		o.Id = &Id
	}
	...
	if Published, ok := EvaluationformsearchresponseMap["published"].(bool); ok {
		o.Published = &Published
	}
    
	if ContextId, ok := EvaluationformsearchresponseMap["contextId"].(string); ok {
		o.ContextId = &ContextId
	}
    
	if QuestionGroups, ok := EvaluationformsearchresponseMap["questionGroups"].([]interface{}); ok {
		QuestionGroupsString, _ := json.Marshal(QuestionGroups)
		json.Unmarshal(QuestionGroupsString, &o.QuestionGroups)
	}
	
	if WeightMode, ok := EvaluationformsearchresponseMap["weightMode"].(string); ok {
		o.WeightMode = &WeightMode
	}
    
	if EvaluationSettings, ok := EvaluationformsearchresponseMap["evaluationSettings"].(map[string]interface{}); ok {
		EvaluationSettingsString, _ := json.Marshal(EvaluationSettings)
		json.Unmarshal(EvaluationSettingsString, &o.EvaluationSettings)
	}
	
	if PublishedVersions, ok := EvaluationformsearchresponseMap["publishedVersions"].(map[string]interface{}); ok {
		PublishedVersionsString, _ := json.Marshal(PublishedVersions)
		json.Unmarshal(PublishedVersionsString, &o.PublishedVersions)
	}
	
	if LatestVersionFormName, ok := EvaluationformsearchresponseMap["latestVersionFormName"].(string); ok {
		o.LatestVersionFormName = &LatestVersionFormName
	}
    
	if AiScoring, ok := EvaluationformsearchresponseMap["aiScoring"].(map[string]interface{}); ok {
		AiScoringString, _ := json.Marshal(AiScoring)
		json.Unmarshal(AiScoringString, &o.AiScoring)
	}
	
	if Dialect, ok := EvaluationformsearchresponseMap["dialect"].(string); ok {
		o.Dialect = &Dialect
	}
    
	if SelfUri, ok := EvaluationformsearchresponseMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

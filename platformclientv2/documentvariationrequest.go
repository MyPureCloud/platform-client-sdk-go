...
type Documentvariationrequest struct { 
	...
	Body *Documentbodyrequest `json:"body,omitempty"`
...
func (o *Documentvariationrequest) SetField(field string, fieldValue interface{}) {
...
func (o Documentvariationrequest) MarshalJSON() ([]byte, error) {
...
func (o *Documentvariationrequest) UnmarshalJSON(b []byte) error {
	var DocumentvariationrequestMap map[string]interface{}
	err := json.Unmarshal(b, &DocumentvariationrequestMap)
	if err != nil {
		return err
	}
	
	if Id, ok := DocumentvariationrequestMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if dateCreatedString, ok := DocumentvariationrequestMap["dateCreated"].(string); ok {
		DateCreated, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCreatedString)
		o.DateCreated = &DateCreated
	}
	
	if dateModifiedString, ok := DocumentvariationrequestMap["dateModified"].(string); ok {
		DateModified, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateModifiedString)
		o.DateModified = &DateModified
	}
	
	if DocumentVersion, ok := DocumentvariationrequestMap["documentVersion"].(map[string]interface{}); ok {
		DocumentVersionString, _ := json.Marshal(DocumentVersion)
		json.Unmarshal(DocumentVersionString, &o.DocumentVersion)
	}
	
	if Contexts, ok := DocumentvariationrequestMap["contexts"].([]interface{}); ok {
		ContextsString, _ := json.Marshal(Contexts)
		json.Unmarshal(ContextsString, &o.Contexts)
	}
	
	if Document, ok := DocumentvariationrequestMap["document"].(map[string]interface{}); ok {
		DocumentString, _ := json.Marshal(Document)
		json.Unmarshal(DocumentString, &o.Document)
	}
	
	if Priority, ok := DocumentvariationrequestMap["priority"].(float64); ok {
		PriorityInt := int(Priority)
		o.Priority = &PriorityInt
	}
	
	if Name, ok := DocumentvariationrequestMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Body, ok := DocumentvariationrequestMap["body"].(map[string]interface{}); ok {
		BodyString, _ := json.Marshal(Body)
		json.Unmarshal(BodyString, &o.Body)
	}
	
	if SelfUri, ok := DocumentvariationrequestMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

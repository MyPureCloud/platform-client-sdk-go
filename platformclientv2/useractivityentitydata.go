...
func (o *Useractivityentitydata) SetField(field string, fieldValue interface{}) {
...
func (o Useractivityentitydata) MarshalJSON() ([]byte, error) {
...
func (o *Useractivityentitydata) UnmarshalJSON(b []byte) error {
	var UseractivityentitydataMap map[string]interface{}
	err := json.Unmarshal(b, &UseractivityentitydataMap)
	if err != nil {
		return err
	}
	
	if activityDateString, ok := UseractivityentitydataMap["activityDate"].(string); ok {
		ActivityDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", activityDateString)
		o.ActivityDate = &ActivityDate
	}
	
	if OrganizationPresenceId, ok := UseractivityentitydataMap["organizationPresenceId"].(string); ok {
		o.OrganizationPresenceId = &OrganizationPresenceId
	}
    
	if presenceDateString, ok := UseractivityentitydataMap["presenceDate"].(string); ok {
		PresenceDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", presenceDateString)
		o.PresenceDate = &PresenceDate
	}
	
	if QueueId, ok := UseractivityentitydataMap["queueId"].(string); ok {
		o.QueueId = &QueueId
	}
    
	if QueueMembershipStatus, ok := UseractivityentitydataMap["queueMembershipStatus"].(string); ok {
		o.QueueMembershipStatus = &QueueMembershipStatus
	}
    
	if RoutingStatus, ok := UseractivityentitydataMap["routingStatus"].(string); ok {
		o.RoutingStatus = &RoutingStatus
	}
    
	if routingStatusDateString, ok := UseractivityentitydataMap["routingStatusDate"].(string); ok {
		RoutingStatusDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", routingStatusDateString)
		o.RoutingStatusDate = &RoutingStatusDate
	}
	
	if SystemPresence, ok := UseractivityentitydataMap["systemPresence"].(string); ok {
		o.SystemPresence = &SystemPresence
	}
    
	if TeamId, ok := UseractivityentitydataMap["teamId"].(string); ok {
		o.TeamId = &TeamId
	}
    
	if UserId, ok := UseractivityentitydataMap["userId"].(string); ok {
		o.UserId = &UserId
	}

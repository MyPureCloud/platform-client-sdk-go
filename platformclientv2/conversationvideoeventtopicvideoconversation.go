...
type Conversationvideoeventtopicvideoconversation struct { 
	...
	// Divisions
	Divisions *[]Conversationvideoeventtopicconversationdivisionmembership `json:"divisions,omitempty"`
}

// SetField uses reflection to set a field on the model if the model has a property SetFieldNames, and triggers custom JSON serialization logic to only serialize properties that have been set using this function.
func (o *Conversationvideoeventtopicvideoconversation) SetField(field string, fieldValue interface{}) {
...
func (o *Conversationvideoeventtopicvideoconversation) UnmarshalJSON(b []byte) error {
	...
	if AccessAttributes, ok := ConversationvideoeventtopicvideoconversationMap["accessAttributes"].([]interface{}); ok {
		AccessAttributesString, _ := json.Marshal(AccessAttributes)
		json.Unmarshal(AccessAttributesString, &o.AccessAttributes)
	}
	
	if inactivityTimeoutString, ok := ConversationvideoeventtopicvideoconversationMap["inactivityTimeout"].(string); ok {
		InactivityTimeout, _ := time.Parse("2006-01-02T15:04:05.999999Z", inactivityTimeoutString)
		o.InactivityTimeout = &InactivityTimeout
	}
	
	if Divisions, ok := ConversationvideoeventtopicvideoconversationMap["divisions"].([]interface{}); ok {

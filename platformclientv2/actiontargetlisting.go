...
func (o *Actiontargetlisting) SetField(field string, fieldValue interface{}) {
...
func (o *Actiontargetlisting) UnmarshalJSON(b []byte) error {
	var ActiontargetlistingMap map[string]interface{}
	err := json.Unmarshal(b, &ActiontargetlistingMap)
	if err != nil {
		return err
	}
	
	if Entities, ok := ActiontargetlistingMap["entities"].([]interface{}); ok {
		EntitiesString, _ := json.Marshal(Entities)
		json.Unmarshal(EntitiesString, &o.Entities)
	}
	
	if PageSize, ok := ActiontargetlistingMap["pageSize"].(float64); ok {
		PageSizeInt := int(PageSize)
		o.PageSize = &PageSizeInt
	}
	
	if PageNumber, ok := ActiontargetlistingMap["pageNumber"].(float64); ok {
		PageNumberInt := int(PageNumber)
		o.PageNumber = &PageNumberInt
	}
	
	if Total, ok := ActiontargetlistingMap["total"].(float64); ok {
		TotalInt := int(Total)
		o.Total = &TotalInt
	}
	...
	if SelfUri, ok := ActiontargetlistingMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

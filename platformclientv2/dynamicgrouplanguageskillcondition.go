...
type Dynamicgrouplanguageskillcondition struct { 
	...
	ChildConditions *[]Dynamicgroupskillcondition `json:"childConditions,omitempty"`
}

// SetField uses reflection to set a field on the model if the model has a property SetFieldNames, and triggers custom JSON serialization logic to only serialize properties that have been set using this function.
func (o *Dynamicgrouplanguageskillcondition) SetField(field string, fieldValue interface{}) {
...
func (o Dynamicgrouplanguageskillcondition) MarshalJSON() ([]byte, error) {
...
func (o *Dynamicgrouplanguageskillcondition) UnmarshalJSON(b []byte) error {
	var DynamicgrouplanguageskillconditionMap map[string]interface{}
	err := json.Unmarshal(b, &DynamicgrouplanguageskillconditionMap)
	if err != nil {
		return err
	}
	
	if LanguageSkill, ok := DynamicgrouplanguageskillconditionMap["languageSkill"].(string); ok {
		o.LanguageSkill = &LanguageSkill
	}
    
	if Comparator, ok := DynamicgrouplanguageskillconditionMap["comparator"].(string); ok {
		o.Comparator = &Comparator
	}
    
	if Proficiency, ok := DynamicgrouplanguageskillconditionMap["proficiency"].(float64); ok {
		ProficiencyInt := int(Proficiency)
		o.Proficiency = &ProficiencyInt
	}
	
	if ChildConditions, ok := DynamicgrouplanguageskillconditionMap["childConditions"].([]interface{}); ok {

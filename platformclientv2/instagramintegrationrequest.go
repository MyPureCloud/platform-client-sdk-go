...
func (o *Instagramintegrationrequest) SetField(field string, fieldValue interface{}) {
...
func (o Instagramintegrationrequest) MarshalJSON() ([]byte, error) {
...
func (o *Instagramintegrationrequest) UnmarshalJSON(b []byte) error {
	var InstagramintegrationrequestMap map[string]interface{}
	err := json.Unmarshal(b, &InstagramintegrationrequestMap)
	if err != nil {
		return err
	}
	
	if Id, ok := InstagramintegrationrequestMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := InstagramintegrationrequestMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if SupportedContent, ok := InstagramintegrationrequestMap["supportedContent"].(map[string]interface{}); ok {
		SupportedContentString, _ := json.Marshal(SupportedContent)
		json.Unmarshal(SupportedContentString, &o.SupportedContent)
	}
	
	if MessagingSetting, ok := InstagramintegrationrequestMap["messagingSetting"].(map[string]interface{}); ok {
		MessagingSettingString, _ := json.Marshal(MessagingSetting)
		json.Unmarshal(MessagingSettingString, &o.MessagingSetting)
	}
	
	if PageAccessToken, ok := InstagramintegrationrequestMap["pageAccessToken"].(string); ok {
		o.PageAccessToken = &PageAccessToken
	}
    
	if UserAccessToken, ok := InstagramintegrationrequestMap["userAccessToken"].(string); ok {
		o.UserAccessToken = &UserAccessToken
	}
    
	if PageId, ok := InstagramintegrationrequestMap["pageId"].(string); ok {
		o.PageId = &PageId
	}
    
	if AppId, ok := InstagramintegrationrequestMap["appId"].(string); ok {
		o.AppId = &AppId
	}
    
	if AppSecret, ok := InstagramintegrationrequestMap["appSecret"].(string); ok {
		o.AppSecret = &AppSecret
	}
    
	if SelfUri, ok := InstagramintegrationrequestMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

...
func (o *Workdayperiod) SetField(field string, fieldValue interface{}) {
...
func (o Workdayperiod) MarshalJSON() ([]byte, error) {
...
func (o *Workdayperiod) UnmarshalJSON(b []byte) error {
	var WorkdayperiodMap map[string]interface{}
	err := json.Unmarshal(b, &WorkdayperiodMap)
	if err != nil {
		return err
	}
	
	if dateStartWorkdayString, ok := WorkdayperiodMap["dateStartWorkday"].(string); ok {
		DateStartWorkday, _ := time.Parse("2006-01-02", dateStartWorkdayString)
		o.DateStartWorkday = &DateStartWorkday
	}
	
	if dateEndWorkdayString, ok := WorkdayperiodMap["dateEndWorkday"].(string); ok {

...
func (o *Emailuserestablishedevent) SetField(field string, fieldValue interface{}) {
...
func (o Emailuserestablishedevent) MarshalJSON() ([]byte, error) {
...
func (o *Emailuserestablishedevent) UnmarshalJSON(b []byte) error {
	var EmailuserestablishedeventMap map[string]interface{}
	err := json.Unmarshal(b, &EmailuserestablishedeventMap)
	if err != nil {
		return err
	}
	
	if EventId, ok := EmailuserestablishedeventMap["eventId"].(string); ok {
		o.EventId = &EventId
	}
    
	if eventDateTimeString, ok := EmailuserestablishedeventMap["eventDateTime"].(string); ok {
		EventDateTime, _ := time.Parse("2006-01-02T15:04:05.999999Z", eventDateTimeString)
		o.EventDateTime = &EventDateTime
	}
	
	if ConversationId, ok := EmailuserestablishedeventMap["conversationId"].(string); ok {
		o.ConversationId = &ConversationId
	}
    
	if CommunicationId, ok := EmailuserestablishedeventMap["communicationId"].(string); ok {
		o.CommunicationId = &CommunicationId
	}
    
	if UserId, ok := EmailuserestablishedeventMap["userId"].(string); ok {
		o.UserId = &UserId
	}
    
	if QueueId, ok := EmailuserestablishedeventMap["queueId"].(string); ok {
		o.QueueId = &QueueId
	}
    
	if AfterCallWorkRequired, ok := EmailuserestablishedeventMap["afterCallWorkRequired"].(bool); ok {
		o.AfterCallWorkRequired = &AfterCallWorkRequired
	}
    
	if InitialConfiguration, ok := EmailuserestablishedeventMap["initialConfiguration"].(map[string]interface{}); ok {
	...
	if SourceConfiguration, ok := EmailuserestablishedeventMap["sourceConfiguration"].(map[string]interface{}); ok {

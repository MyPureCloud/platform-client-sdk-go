...
func (o *Agentscheduleshiftactivity) SetField(field string, fieldValue interface{}) {
...
func (o Agentscheduleshiftactivity) MarshalJSON() ([]byte, error) {
...
func (o *Agentscheduleshiftactivity) UnmarshalJSON(b []byte) error {
	var AgentscheduleshiftactivityMap map[string]interface{}
	err := json.Unmarshal(b, &AgentscheduleshiftactivityMap)
	if err != nil {
		return err
	}
	
	if ActivityCategory, ok := AgentscheduleshiftactivityMap["activityCategory"].(string); ok {
		o.ActivityCategory = &ActivityCategory
	}
    
	if StartOffsetMinutes, ok := AgentscheduleshiftactivityMap["startOffsetMinutes"].(float64); ok {
		StartOffsetMinutesInt := int(StartOffsetMinutes)
		o.StartOffsetMinutes = &StartOffsetMinutesInt
	}
	
	if LengthMinutes, ok := AgentscheduleshiftactivityMap["lengthMinutes"].(float64); ok {

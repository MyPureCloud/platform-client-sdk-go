...
func (o *Estimateavailablefulldaytimeoffresponse) SetField(field string, fieldValue interface{}) {
...
func (o Estimateavailablefulldaytimeoffresponse) MarshalJSON() ([]byte, error) {
...
func (o *Estimateavailablefulldaytimeoffresponse) UnmarshalJSON(b []byte) error {
	var EstimateavailablefulldaytimeoffresponseMap map[string]interface{}
	err := json.Unmarshal(b, &EstimateavailablefulldaytimeoffresponseMap)
	if err != nil {
		return err
	}
	
	if dateString, ok := EstimateavailablefulldaytimeoffresponseMap["date"].(string); ok {
		Date, _ := time.Parse("2006-01-02", dateString)
		o.Date = &Date
	}
	
	if DurationMinutes, ok := EstimateavailablefulldaytimeoffresponseMap["durationMinutes"].(float64); ok {
		DurationMinutesInt := int(DurationMinutes)
		o.DurationMinutes = &DurationMinutesInt
	}
	
	if PayableMinutes, ok := EstimateavailablefulldaytimeoffresponseMap["payableMinutes"].(float64); ok {
		PayableMinutesInt := int(PayableMinutes)
		o.PayableMinutes = &PayableMinutesInt
	}
	
	if Flexible, ok := EstimateavailablefulldaytimeoffresponseMap["flexible"].(bool); ok {
		o.Flexible = &Flexible
	}
    
	if OverrideDateType, ok := EstimateavailablefulldaytimeoffresponseMap["overrideDateType"].(string); ok {

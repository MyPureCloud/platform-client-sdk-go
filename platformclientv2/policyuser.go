...
func (o *Policyuser) SetField(field string, fieldValue interface{}) {
...
func (o *Policyuser) UnmarshalJSON(b []byte) error {
	var PolicyuserMap map[string]interface{}
	err := json.Unmarshal(b, &PolicyuserMap)
	if err != nil {
		return err
	}
	
	if Id, ok := PolicyuserMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := PolicyuserMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Division, ok := PolicyuserMap["division"].(map[string]interface{}); ok {
		DivisionString, _ := json.Marshal(Division)
		json.Unmarshal(DivisionString, &o.Division)
	}
	
	if Chat, ok := PolicyuserMap["chat"].(map[string]interface{}); ok {
		ChatString, _ := json.Marshal(Chat)
		json.Unmarshal(ChatString, &o.Chat)
	}
	
	if Department, ok := PolicyuserMap["department"].(string); ok {
		o.Department = &Department
	}
    
	if Email, ok := PolicyuserMap["email"].(string); ok {
		o.Email = &Email
	}
    
	if PrimaryContactInfo, ok := PolicyuserMap["primaryContactInfo"].([]interface{}); ok {
		PrimaryContactInfoString, _ := json.Marshal(PrimaryContactInfo)
		json.Unmarshal(PrimaryContactInfoString, &o.PrimaryContactInfo)
	}
	
	if Addresses, ok := PolicyuserMap["addresses"].([]interface{}); ok {
		AddressesString, _ := json.Marshal(Addresses)
		json.Unmarshal(AddressesString, &o.Addresses)
	}
	
	if State, ok := PolicyuserMap["state"].(string); ok {
		o.State = &State
	}
    
	if Title, ok := PolicyuserMap["title"].(string); ok {
		o.Title = &Title
	}
    
	if Username, ok := PolicyuserMap["username"].(string); ok {
		o.Username = &Username
	}
    
	if Manager, ok := PolicyuserMap["manager"].(map[string]interface{}); ok {
		ManagerString, _ := json.Marshal(Manager)
		json.Unmarshal(ManagerString, &o.Manager)
	}
	
	if Images, ok := PolicyuserMap["images"].([]interface{}); ok {
		ImagesString, _ := json.Marshal(Images)
		json.Unmarshal(ImagesString, &o.Images)
	}
	
	if Version, ok := PolicyuserMap["version"].(float64); ok {
		VersionInt := int(Version)
		o.Version = &VersionInt
	}
	
	if Certifications, ok := PolicyuserMap["certifications"].([]interface{}); ok {
		CertificationsString, _ := json.Marshal(Certifications)
		json.Unmarshal(CertificationsString, &o.Certifications)
	}
	
	if Biography, ok := PolicyuserMap["biography"].(map[string]interface{}); ok {
		BiographyString, _ := json.Marshal(Biography)
		json.Unmarshal(BiographyString, &o.Biography)
	}
	
	if EmployerInfo, ok := PolicyuserMap["employerInfo"].(map[string]interface{}); ok {
		EmployerInfoString, _ := json.Marshal(EmployerInfo)
		json.Unmarshal(EmployerInfoString, &o.EmployerInfo)
	}
	
	if PreferredName, ok := PolicyuserMap["preferredName"].(string); ok {
		o.PreferredName = &PreferredName
	}
    
	if RoutingStatus, ok := PolicyuserMap["routingStatus"].(map[string]interface{}); ok {
		RoutingStatusString, _ := json.Marshal(RoutingStatus)
		json.Unmarshal(RoutingStatusString, &o.RoutingStatus)
	}
	
	if Presence, ok := PolicyuserMap["presence"].(map[string]interface{}); ok {
		PresenceString, _ := json.Marshal(Presence)
		json.Unmarshal(PresenceString, &o.Presence)
	}
	
	if IntegrationPresence, ok := PolicyuserMap["integrationPresence"].(map[string]interface{}); ok {
	...
	if ConversationSummary, ok := PolicyuserMap["conversationSummary"].(map[string]interface{}); ok {

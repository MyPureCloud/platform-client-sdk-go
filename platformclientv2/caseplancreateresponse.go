...
func (o *Caseplancreateresponse) SetField(field string, fieldValue interface{}) {
...
func (o Caseplancreateresponse) MarshalJSON() ([]byte, error) {
...
func (o *Caseplancreateresponse) UnmarshalJSON(b []byte) error {
	var CaseplancreateresponseMap map[string]interface{}
	err := json.Unmarshal(b, &CaseplancreateresponseMap)
	if err != nil {
		return err
	}
	
	if Id, ok := CaseplancreateresponseMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := CaseplancreateresponseMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Division, ok := CaseplancreateresponseMap["division"].(map[string]interface{}); ok {
		DivisionString, _ := json.Marshal(Division)
		json.Unmarshal(DivisionString, &o.Division)
	}
	
	if Description, ok := CaseplancreateresponseMap["description"].(string); ok {
		o.Description = &Description
	}
    
	if ReferencePrefix, ok := CaseplancreateresponseMap["referencePrefix"].(string); ok {
		o.ReferencePrefix = &ReferencePrefix
	}
    
	if DefaultDueDurationInSeconds, ok := CaseplancreateresponseMap["defaultDueDurationInSeconds"].(float64); ok {
	...
	if DefaultTtlSeconds, ok := CaseplancreateresponseMap["defaultTtlSeconds"].(float64); ok {
		DefaultTtlSecondsInt := int(DefaultTtlSeconds)
		o.DefaultTtlSeconds = &DefaultTtlSecondsInt
	}
	
	if DefaultCaseOwner, ok := CaseplancreateresponseMap["defaultCaseOwner"].(map[string]interface{}); ok {
		DefaultCaseOwnerString, _ := json.Marshal(DefaultCaseOwner)
		json.Unmarshal(DefaultCaseOwnerString, &o.DefaultCaseOwner)
	}
	
	if Latest, ok := CaseplancreateresponseMap["latest"].(float64); ok {
		LatestInt := int(Latest)
		o.Latest = &LatestInt
	}
	
	if Published, ok := CaseplancreateresponseMap["published"].(float64); ok {
		PublishedInt := int(Published)
		o.Published = &PublishedInt
	}
	
	if dateCreatedString, ok := CaseplancreateresponseMap["dateCreated"].(string); ok {
		DateCreated, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCreatedString)
		o.DateCreated = &DateCreated
	}
	
	if dateModifiedString, ok := CaseplancreateresponseMap["dateModified"].(string); ok {
		DateModified, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateModifiedString)
		o.DateModified = &DateModified
	}
	
	if datePublishedString, ok := CaseplancreateresponseMap["datePublished"].(string); ok {
		DatePublished, _ := time.Parse("2006-01-02T15:04:05.999999Z", datePublishedString)
		o.DatePublished = &DatePublished
	}
	
	if ModifiedBy, ok := CaseplancreateresponseMap["modifiedBy"].(map[string]interface{}); ok {
		ModifiedByString, _ := json.Marshal(ModifiedBy)
		json.Unmarshal(ModifiedByString, &o.ModifiedBy)
	}
	
	if CustomerIntent, ok := CaseplancreateresponseMap["customerIntent"].(map[string]interface{}); ok {
		CustomerIntentString, _ := json.Marshal(CustomerIntent)
		json.Unmarshal(CustomerIntentString, &o.CustomerIntent)
	}
	
	if VersionState, ok := CaseplancreateresponseMap["versionState"].(string); ok {
		o.VersionState = &VersionState
	}
    
	if DataSchemas, ok := CaseplancreateresponseMap["dataSchemas"].([]interface{}); ok {
		DataSchemasString, _ := json.Marshal(DataSchemas)
		json.Unmarshal(DataSchemasString, &o.DataSchemas)
	}
	
	if IntakeSettings, ok := CaseplancreateresponseMap["intakeSettings"].([]interface{}); ok {
		IntakeSettingsString, _ := json.Marshal(IntakeSettings)
		json.Unmarshal(IntakeSettingsString, &o.IntakeSettings)
	}
	
	if SelfUri, ok := CaseplancreateresponseMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

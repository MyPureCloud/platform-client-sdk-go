...
type Taskmanagementasyncaggregationquery struct { 
	...
	Filter *Taskmanagementaggregatequeryfilter `json:"filter,omitempty"`
	...
	Views *[]Taskmanagementaggregationview `json:"views,omitempty"`
...
func (o *Taskmanagementasyncaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Taskmanagementasyncaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Taskmanagementasyncaggregationquery) UnmarshalJSON(b []byte) error {
	var TaskmanagementasyncaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &TaskmanagementasyncaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := TaskmanagementasyncaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := TaskmanagementasyncaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := TaskmanagementasyncaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := TaskmanagementasyncaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := TaskmanagementasyncaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := TaskmanagementasyncaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := TaskmanagementasyncaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := TaskmanagementasyncaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := TaskmanagementasyncaggregationqueryMap["alternateTimeDimension"].(string); ok {
		o.AlternateTimeDimension = &AlternateTimeDimension
	}
    
	if PageSize, ok := TaskmanagementasyncaggregationqueryMap["pageSize"].(float64); ok {

...
type Workbinqueryrequest struct { 
	...
	// Sort - Sort
	Sort *Workbinquerysort `json:"sort,omitempty"`
}

// SetField uses reflection to set a field on the model if the model has a property SetFieldNames, and triggers custom JSON serialization logic to only serialize properties that have been set using this function.
func (o *Workbinqueryrequest) SetField(field string, fieldValue interface{}) {
...
func (o Workbinqueryrequest) MarshalJSON() ([]byte, error) {
...
func (o *Workbinqueryrequest) UnmarshalJSON(b []byte) error {
	var WorkbinqueryrequestMap map[string]interface{}
	err := json.Unmarshal(b, &WorkbinqueryrequestMap)
	if err != nil {
		return err
	}
	
	if PageSize, ok := WorkbinqueryrequestMap["pageSize"].(float64); ok {
		PageSizeInt := int(PageSize)
		o.PageSize = &PageSizeInt
	}
	
	if VarSelect, ok := WorkbinqueryrequestMap["select"].(string); ok {
		o.VarSelect = &VarSelect
	}
    
	if Filters, ok := WorkbinqueryrequestMap["filters"].([]interface{}); ok {
		FiltersString, _ := json.Marshal(Filters)
		json.Unmarshal(FiltersString, &o.Filters)
	}
	
	if Attributes, ok := WorkbinqueryrequestMap["attributes"].([]interface{}); ok {
		AttributesString, _ := json.Marshal(Attributes)
		json.Unmarshal(AttributesString, &o.Attributes)
	}
	
	if After, ok := WorkbinqueryrequestMap["after"].(string); ok {
		o.After = &After
	}
    
	if Sort, ok := WorkbinqueryrequestMap["sort"].(map[string]interface{}); ok {

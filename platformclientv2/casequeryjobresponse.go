...
type Casequeryjobresponse struct { 
	...
	VarError *Casequeryjoberror `json:"error,omitempty"`
...
func (o *Casequeryjobresponse) SetField(field string, fieldValue interface{}) {
...
func (o Casequeryjobresponse) MarshalJSON() ([]byte, error) {
...
func (o *Casequeryjobresponse) UnmarshalJSON(b []byte) error {
	var CasequeryjobresponseMap map[string]interface{}
	err := json.Unmarshal(b, &CasequeryjobresponseMap)
	if err != nil {
		return err
	}
	
	if Id, ok := CasequeryjobresponseMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if State, ok := CasequeryjobresponseMap["state"].(string); ok {
		o.State = &State
	}
    
	if dateStartedString, ok := CasequeryjobresponseMap["dateStarted"].(string); ok {
		DateStarted, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateStartedString)
		o.DateStarted = &DateStarted
	}
	
	if dateFinishedString, ok := CasequeryjobresponseMap["dateFinished"].(string); ok {
		DateFinished, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateFinishedString)
		o.DateFinished = &DateFinished
	}
	
	if VarError, ok := CasequeryjobresponseMap["error"].(map[string]interface{}); ok {
		VarErrorString, _ := json.Marshal(VarError)
		json.Unmarshal(VarErrorString, &o.VarError)
	}
	
	if SelfUri, ok := CasequeryjobresponseMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

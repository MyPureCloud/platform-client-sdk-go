...
func (o *Eventaction) SetField(field string, fieldValue interface{}) {
...
func (o *Eventaction) UnmarshalJSON(b []byte) error {
	var EventactionMap map[string]interface{}
	err := json.Unmarshal(b, &EventactionMap)
	if err != nil {
		return err
	}
	
	if Id, ok := EventactionMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if State, ok := EventactionMap["state"].(string); ok {
		o.State = &State
	}
    
	if MediaType, ok := EventactionMap["mediaType"].(string); ok {
		o.MediaType = &MediaType
	}
    
	if Prompt, ok := EventactionMap["prompt"].(string); ok {
		o.Prompt = &Prompt
	}
    
	if createdDateString, ok := EventactionMap["createdDate"].(string); ok {

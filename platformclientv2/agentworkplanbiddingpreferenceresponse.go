...
type Agentworkplanbiddingpreferenceresponse struct { 
	...
	AgentWorkPlanBidPreferences *[]Agentworkplanbiddingpreference `json:"agentWorkPlanBidPreferences,omitempty"`
...
func (o *Agentworkplanbiddingpreferenceresponse) SetField(field string, fieldValue interface{}) {
...
func (o Agentworkplanbiddingpreferenceresponse) MarshalJSON() ([]byte, error) {
...
func (o *Agentworkplanbiddingpreferenceresponse) UnmarshalJSON(b []byte) error {
	var AgentworkplanbiddingpreferenceresponseMap map[string]interface{}
	err := json.Unmarshal(b, &AgentworkplanbiddingpreferenceresponseMap)
	if err != nil {
		return err
	}
	
	if Id, ok := AgentworkplanbiddingpreferenceresponseMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Submitted, ok := AgentworkplanbiddingpreferenceresponseMap["submitted"].(bool); ok {
		o.Submitted = &Submitted
	}
    
	if AssignedWorkPlan, ok := AgentworkplanbiddingpreferenceresponseMap["assignedWorkPlan"].(map[string]interface{}); ok {
		AssignedWorkPlanString, _ := json.Marshal(AssignedWorkPlan)
		json.Unmarshal(AssignedWorkPlanString, &o.AssignedWorkPlan)
	}
	
	if OverriddenWorkPlan, ok := AgentworkplanbiddingpreferenceresponseMap["overriddenWorkPlan"].(map[string]interface{}); ok {
		OverriddenWorkPlanString, _ := json.Marshal(OverriddenWorkPlan)
		json.Unmarshal(OverriddenWorkPlanString, &o.OverriddenWorkPlan)
	}
	
	if OverrideReason, ok := AgentworkplanbiddingpreferenceresponseMap["overrideReason"].(string); ok {
		o.OverrideReason = &OverrideReason
	}
    
	if AgentWorkPlanBidPreferences, ok := AgentworkplanbiddingpreferenceresponseMap["agentWorkPlanBidPreferences"].([]interface{}); ok {
	...
	if SelfUri, ok := AgentworkplanbiddingpreferenceresponseMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

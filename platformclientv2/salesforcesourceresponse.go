...
type Salesforcesourceresponse struct { 
	...
	Settings *Salesforcesettings `json:"settings,omitempty"`
...
func (o *Salesforcesourceresponse) SetField(field string, fieldValue interface{}) {
...
func (o Salesforcesourceresponse) MarshalJSON() ([]byte, error) {
...
func (o *Salesforcesourceresponse) UnmarshalJSON(b []byte) error {
	var SalesforcesourceresponseMap map[string]interface{}
	err := json.Unmarshal(b, &SalesforcesourceresponseMap)
	if err != nil {
		return err
	}
	
	if Id, ok := SalesforcesourceresponseMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := SalesforcesourceresponseMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if dateCreatedString, ok := SalesforcesourceresponseMap["dateCreated"].(string); ok {
		DateCreated, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCreatedString)
		o.DateCreated = &DateCreated
	}
	
	if dateModifiedString, ok := SalesforcesourceresponseMap["dateModified"].(string); ok {
		DateModified, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateModifiedString)
		o.DateModified = &DateModified
	}
	
	if VarType, ok := SalesforcesourceresponseMap["type"].(string); ok {
		o.VarType = &VarType
	}
    
	if Integration, ok := SalesforcesourceresponseMap["integration"].(map[string]interface{}); ok {
		IntegrationString, _ := json.Marshal(Integration)
		json.Unmarshal(IntegrationString, &o.Integration)
	}
	
	if SchedulePeriod, ok := SalesforcesourceresponseMap["schedulePeriod"].(float64); ok {
		SchedulePeriodInt := int(SchedulePeriod)
		o.SchedulePeriod = &SchedulePeriodInt
	}
	
	if LastSync, ok := SalesforcesourceresponseMap["lastSync"].(map[string]interface{}); ok {
		LastSyncString, _ := json.Marshal(LastSync)
		json.Unmarshal(LastSyncString, &o.LastSync)
	}
	
	if Settings, ok := SalesforcesourceresponseMap["settings"].(map[string]interface{}); ok {
		SettingsString, _ := json.Marshal(Settings)
		json.Unmarshal(SettingsString, &o.Settings)
	}
	
	if SelfUri, ok := SalesforcesourceresponseMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

...
type Workitemqueryjobresponse struct { 
	...
	VarError *Workitemqueryjoberror `json:"error,omitempty"`
...
func (o *Workitemqueryjobresponse) SetField(field string, fieldValue interface{}) {
...
func (o Workitemqueryjobresponse) MarshalJSON() ([]byte, error) {
	...
	if len(o.SetFieldNames) > 0 {
		...
		dateTimeFields := []string{ "DateStarted","DateFinished", }
...
func (o *Workitemqueryjobresponse) UnmarshalJSON(b []byte) error {
	var WorkitemqueryjobresponseMap map[string]interface{}
	err := json.Unmarshal(b, &WorkitemqueryjobresponseMap)
	if err != nil {
		return err
	}
	
	if Id, ok := WorkitemqueryjobresponseMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if State, ok := WorkitemqueryjobresponseMap["state"].(string); ok {
		o.State = &State
	}
    
	if dateStartedString, ok := WorkitemqueryjobresponseMap["dateStarted"].(string); ok {
		DateStarted, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateStartedString)
		o.DateStarted = &DateStarted
	}
	
	if dateFinishedString, ok := WorkitemqueryjobresponseMap["dateFinished"].(string); ok {
		DateFinished, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateFinishedString)
		o.DateFinished = &DateFinished
	}
	
	if VarError, ok := WorkitemqueryjobresponseMap["error"].(map[string]interface{}); ok {
		VarErrorString, _ := json.Marshal(VarError)
		json.Unmarshal(VarErrorString, &o.VarError)
	}
	
	if SelfUri, ok := WorkitemqueryjobresponseMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

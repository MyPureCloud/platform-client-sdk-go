...
type Copilotasyncaggregationquery struct { 
	...
	Filter *Copilotaggregatequeryfilter `json:"filter,omitempty"`
	...
	Views *[]Copilotaggregationview `json:"views,omitempty"`
...
func (o *Copilotasyncaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Copilotasyncaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Copilotasyncaggregationquery) UnmarshalJSON(b []byte) error {
	var CopilotasyncaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &CopilotasyncaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := CopilotasyncaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := CopilotasyncaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := CopilotasyncaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := CopilotasyncaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := CopilotasyncaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := CopilotasyncaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := CopilotasyncaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := CopilotasyncaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := CopilotasyncaggregationqueryMap["alternateTimeDimension"].(string); ok {
		o.AlternateTimeDimension = &AlternateTimeDimension
	}
    
	if PageSize, ok := CopilotasyncaggregationqueryMap["pageSize"].(float64); ok {

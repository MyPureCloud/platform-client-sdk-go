...
type Agentqueryopportunityresult struct { 
	...
	EnrollmentCounts *Pendingandapprovedopportunityenrollmentcounts `json:"enrollmentCounts,omitempty"`

	// Enrollment - The agent's enrollment in this opportunity, if enrolled
	Enrollment *Agentopportunityenrollmentresult `json:"enrollment,omitempty"`
...
func (o *Agentqueryopportunityresult) SetField(field string, fieldValue interface{}) {
...
func (o Agentqueryopportunityresult) MarshalJSON() ([]byte, error) {
...
func (o *Agentqueryopportunityresult) UnmarshalJSON(b []byte) error {
	var AgentqueryopportunityresultMap map[string]interface{}
	err := json.Unmarshal(b, &AgentqueryopportunityresultMap)
	if err != nil {
		return err
	}
	
	if Id, ok := AgentqueryopportunityresultMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := AgentqueryopportunityresultMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Description, ok := AgentqueryopportunityresultMap["description"].(string); ok {
		o.Description = &Description
	}
    
	if ActivityCodeId, ok := AgentqueryopportunityresultMap["activityCodeId"].(string); ok {
		o.ActivityCodeId = &ActivityCodeId
	}
    
	if startDateString, ok := AgentqueryopportunityresultMap["startDate"].(string); ok {
		StartDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", startDateString)
		o.StartDate = &StartDate
	}
	
	if endDateString, ok := AgentqueryopportunityresultMap["endDate"].(string); ok {
		EndDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", endDateString)
		o.EndDate = &EndDate
	}
	
	if deadlineDateString, ok := AgentqueryopportunityresultMap["deadlineDate"].(string); ok {
		DeadlineDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", deadlineDateString)
		o.DeadlineDate = &DeadlineDate
	}
	
	if Status, ok := AgentqueryopportunityresultMap["status"].(string); ok {
		o.Status = &Status
	}
    
	if Capacity, ok := AgentqueryopportunityresultMap["capacity"].(float64); ok {
		CapacityInt := int(Capacity)
		o.Capacity = &CapacityInt
	}
	
	if EnrollmentCounts, ok := AgentqueryopportunityresultMap["enrollmentCounts"].(map[string]interface{}); ok {
		EnrollmentCountsString, _ := json.Marshal(EnrollmentCounts)
		json.Unmarshal(EnrollmentCountsString, &o.EnrollmentCounts)
	}
	
	if Enrollment, ok := AgentqueryopportunityresultMap["enrollment"].(map[string]interface{}); ok {
		EnrollmentString, _ := json.Marshal(Enrollment)
		json.Unmarshal(EnrollmentString, &o.Enrollment)
	}
	
	if Metadata, ok := AgentqueryopportunityresultMap["metadata"].(map[string]interface{}); ok {

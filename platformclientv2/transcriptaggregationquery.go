...
func (o *Transcriptaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o *Transcriptaggregationquery) UnmarshalJSON(b []byte) error {
	var TranscriptaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &TranscriptaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := TranscriptaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := TranscriptaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := TranscriptaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := TranscriptaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := TranscriptaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := TranscriptaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := TranscriptaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := TranscriptaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := TranscriptaggregationqueryMap["alternateTimeDimension"].(string); ok {
		o.AlternateTimeDimension = &AlternateTimeDimension
	}
    
	if QueryType, ok := TranscriptaggregationqueryMap["queryType"].(string); ok {
		o.QueryType = &QueryType
	}
    
	if SortMetric, ok := TranscriptaggregationqueryMap["sortMetric"].(map[string]interface{}); ok {
		SortMetricString, _ := json.Marshal(SortMetric)
		json.Unmarshal(SortMetricString, &o.SortMetric)
	}
	
	if Limit, ok := TranscriptaggregationqueryMap["limit"].(float64); ok {
		LimitInt := int(Limit)
		o.Limit = &LimitInt
	}

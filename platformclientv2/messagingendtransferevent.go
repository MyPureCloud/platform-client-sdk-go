...
func (o *Messagingendtransferevent) SetField(field string, fieldValue interface{}) {
...
func (o Messagingendtransferevent) MarshalJSON() ([]byte, error) {
...
func (o *Messagingendtransferevent) UnmarshalJSON(b []byte) error {
	var MessagingendtransfereventMap map[string]interface{}
	err := json.Unmarshal(b, &MessagingendtransfereventMap)
	if err != nil {
		return err
	}
	
	if EventId, ok := MessagingendtransfereventMap["eventId"].(string); ok {
		o.EventId = &EventId
	}
    
	if eventDateTimeString, ok := MessagingendtransfereventMap["eventDateTime"].(string); ok {
		EventDateTime, _ := time.Parse("2006-01-02T15:04:05.999999Z", eventDateTimeString)
		o.EventDateTime = &EventDateTime
	}
	
	if ConversationId, ok := MessagingendtransfereventMap["conversationId"].(string); ok {
		o.ConversationId = &ConversationId
	}
    
	if CommandId, ok := MessagingendtransfereventMap["commandId"].(string); ok {
		o.CommandId = &CommandId
	}
    
	if FinalState, ok := MessagingendtransfereventMap["finalState"].(string); ok {
		o.FinalState = &FinalState
	}
    
	if ObjectCommunicationId, ok := MessagingendtransfereventMap["objectCommunicationId"].(string); ok {

...
func (o *Conversationsummary) SetField(field string, fieldValue interface{}) {
...
func (o *Conversationsummary) UnmarshalJSON(b []byte) error {
	var ConversationsummaryMap map[string]interface{}
	err := json.Unmarshal(b, &ConversationsummaryMap)
	if err != nil {
		return err
	}
	...
	if MediaType, ok := ConversationsummaryMap["mediaType"].(string); ok {
		o.MediaType = &MediaType
	}
    
	if Language, ok := ConversationsummaryMap["language"].(string); ok {
		o.Language = &Language
	}
    
	if PredictedWrapupCodes, ok := ConversationsummaryMap["predictedWrapupCodes"].([]interface{}); ok {
	...
	if EditedSummary, ok := ConversationsummaryMap["editedSummary"].(map[string]interface{}); ok {
		EditedSummaryString, _ := json.Marshal(EditedSummary)
		json.Unmarshal(EditedSummaryString, &o.EditedSummary)
	}
	
	if Reason, ok := ConversationsummaryMap["reason"].(map[string]interface{}); ok {
		ReasonString, _ := json.Marshal(Reason)
		json.Unmarshal(ReasonString, &o.Reason)
	}
	
	if Followup, ok := ConversationsummaryMap["followup"].(map[string]interface{}); ok {
		FollowupString, _ := json.Marshal(Followup)
		json.Unmarshal(FollowupString, &o.Followup)
	}
	
	if Resolution, ok := ConversationsummaryMap["resolution"].(map[string]interface{}); ok {

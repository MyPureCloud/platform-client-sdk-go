...
func (o *Casemanagementaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Casemanagementaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Casemanagementaggregationquery) UnmarshalJSON(b []byte) error {
	var CasemanagementaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &CasemanagementaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := CasemanagementaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := CasemanagementaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := CasemanagementaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := CasemanagementaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := CasemanagementaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := CasemanagementaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := CasemanagementaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := CasemanagementaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := CasemanagementaggregationqueryMap["alternateTimeDimension"].(string); ok {

...
type Flowexecutionasyncaggregationquery struct { 
	...
	Filter *Flowexecutionaggregatequeryfilter `json:"filter,omitempty"`
	...
	Views *[]Flowexecutionaggregationview `json:"views,omitempty"`
...
func (o *Flowexecutionasyncaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Flowexecutionasyncaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Flowexecutionasyncaggregationquery) UnmarshalJSON(b []byte) error {
	var FlowexecutionasyncaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &FlowexecutionasyncaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := FlowexecutionasyncaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := FlowexecutionasyncaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := FlowexecutionasyncaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := FlowexecutionasyncaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := FlowexecutionasyncaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := FlowexecutionasyncaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := FlowexecutionasyncaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := FlowexecutionasyncaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := FlowexecutionasyncaggregationqueryMap["alternateTimeDimension"].(string); ok {
		o.AlternateTimeDimension = &AlternateTimeDimension
	}
    
	if PageSize, ok := FlowexecutionasyncaggregationqueryMap["pageSize"].(float64); ok {

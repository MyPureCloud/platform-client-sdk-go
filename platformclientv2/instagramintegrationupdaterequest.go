...
func (o *Instagramintegrationupdaterequest) SetField(field string, fieldValue interface{}) {
...
func (o Instagramintegrationupdaterequest) MarshalJSON() ([]byte, error) {
...
func (o *Instagramintegrationupdaterequest) UnmarshalJSON(b []byte) error {
	var InstagramintegrationupdaterequestMap map[string]interface{}
	err := json.Unmarshal(b, &InstagramintegrationupdaterequestMap)
	if err != nil {
		return err
	}
	
	if Id, ok := InstagramintegrationupdaterequestMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := InstagramintegrationupdaterequestMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if SupportedContent, ok := InstagramintegrationupdaterequestMap["supportedContent"].(map[string]interface{}); ok {
		SupportedContentString, _ := json.Marshal(SupportedContent)
		json.Unmarshal(SupportedContentString, &o.SupportedContent)
	}
	
	if MessagingSetting, ok := InstagramintegrationupdaterequestMap["messagingSetting"].(map[string]interface{}); ok {
		MessagingSettingString, _ := json.Marshal(MessagingSetting)
		json.Unmarshal(MessagingSettingString, &o.MessagingSetting)
	}
	
	if PageAccessToken, ok := InstagramintegrationupdaterequestMap["pageAccessToken"].(string); ok {
		o.PageAccessToken = &PageAccessToken
	}
    
	if UserAccessToken, ok := InstagramintegrationupdaterequestMap["userAccessToken"].(string); ok {
		o.UserAccessToken = &UserAccessToken
	}
    
	if SelfUri, ok := InstagramintegrationupdaterequestMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

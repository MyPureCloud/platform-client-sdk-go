...
type Notesexport struct { 
	...
	QueryConditions *Notesexportqueryconditions `json:"queryConditions,omitempty"`
...
func (o *Notesexport) SetField(field string, fieldValue interface{}) {
...
func (o Notesexport) MarshalJSON() ([]byte, error) {
...
func (o *Notesexport) UnmarshalJSON(b []byte) error {
	var NotesexportMap map[string]interface{}
	err := json.Unmarshal(b, &NotesexportMap)
	if err != nil {
		return err
	}
	
	if Id, ok := NotesexportMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if DivisionIds, ok := NotesexportMap["divisionIds"].([]interface{}); ok {
		DivisionIdsString, _ := json.Marshal(DivisionIds)
		json.Unmarshal(DivisionIdsString, &o.DivisionIds)
	}
	
	if CreatedBy, ok := NotesexportMap["createdBy"].(map[string]interface{}); ok {
		CreatedByString, _ := json.Marshal(CreatedBy)
		json.Unmarshal(CreatedByString, &o.CreatedBy)
	}
	
	if dateCreatedString, ok := NotesexportMap["dateCreated"].(string); ok {
		DateCreated, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCreatedString)
		o.DateCreated = &DateCreated
	}
	
	if dateCompletionString, ok := NotesexportMap["dateCompletion"].(string); ok {
		DateCompletion, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCompletionString)
		o.DateCompletion = &DateCompletion
	}
	
	if Status, ok := NotesexportMap["status"].(string); ok {
		o.Status = &Status
	}
    
	if DownloadUrl, ok := NotesexportMap["downloadUrl"].(string); ok {
		o.DownloadUrl = &DownloadUrl
	}
    
	if ResultRowCount, ok := NotesexportMap["resultRowCount"].(float64); ok {
		ResultRowCountInt := int(ResultRowCount)
		o.ResultRowCount = &ResultRowCountInt
	}
	
	if QueryConditions, ok := NotesexportMap["queryConditions"].(map[string]interface{}); ok {
		QueryConditionsString, _ := json.Marshal(QueryConditions)
		json.Unmarshal(QueryConditionsString, &o.QueryConditions)
	}
	
	if SelfUri, ok := NotesexportMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

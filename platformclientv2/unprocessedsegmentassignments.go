...
func (o *Unprocessedsegmentassignments) SetField(field string, fieldValue interface{}) {
...
func (o Unprocessedsegmentassignments) MarshalJSON() ([]byte, error) {
...
func (o *Unprocessedsegmentassignments) UnmarshalJSON(b []byte) error {
	var UnprocessedsegmentassignmentsMap map[string]interface{}
	err := json.Unmarshal(b, &UnprocessedsegmentassignmentsMap)
	if err != nil {
		return err
	}
	
	if Assign, ok := UnprocessedsegmentassignmentsMap["assign"].(map[string]interface{}); ok {
		AssignString, _ := json.Marshal(Assign)
		json.Unmarshal(AssignString, &o.Assign)
	}
	
	if Unassign, ok := UnprocessedsegmentassignmentsMap["unassign"].(map[string]interface{}); ok {
		UnassignString, _ := json.Marshal(Unassign)
		json.Unmarshal(UnassignString, &o.Unassign)
	}
	
	if Count, ok := UnprocessedsegmentassignmentsMap["count"].(float64); ok {
		CountInt := int(Count)
		o.Count = &CountInt
	}

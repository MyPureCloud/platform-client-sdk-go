...
type Knowledgeasyncaggregationquery struct { 
	...
	Filter *Knowledgeaggregatequeryfilter `json:"filter,omitempty"`
	...
	Views *[]Knowledgeaggregationview `json:"views,omitempty"`
	...
	SortMetric *Knowledgeaggregationsort `json:"sortMetric,omitempty"`
...
func (o *Knowledgeasyncaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Knowledgeasyncaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Knowledgeasyncaggregationquery) UnmarshalJSON(b []byte) error {
	var KnowledgeasyncaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &KnowledgeasyncaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := KnowledgeasyncaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := KnowledgeasyncaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := KnowledgeasyncaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := KnowledgeasyncaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := KnowledgeasyncaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := KnowledgeasyncaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := KnowledgeasyncaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := KnowledgeasyncaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := KnowledgeasyncaggregationqueryMap["alternateTimeDimension"].(string); ok {
		o.AlternateTimeDimension = &AlternateTimeDimension
	}
    
	if QueryType, ok := KnowledgeasyncaggregationqueryMap["queryType"].(string); ok {
		o.QueryType = &QueryType
	}
    
	if SortMetric, ok := KnowledgeasyncaggregationqueryMap["sortMetric"].(map[string]interface{}); ok {
		SortMetricString, _ := json.Marshal(SortMetric)
		json.Unmarshal(SortMetricString, &o.SortMetric)
	}
	
	if Limit, ok := KnowledgeasyncaggregationqueryMap["limit"].(float64); ok {
		LimitInt := int(Limit)
		o.Limit = &LimitInt
	}
	
	if PageSize, ok := KnowledgeasyncaggregationqueryMap["pageSize"].(float64); ok {

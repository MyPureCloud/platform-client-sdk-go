...
func (o *Opendataingestionruleversionresponse) SetField(field string, fieldValue interface{}) {
...
func (o Opendataingestionruleversionresponse) MarshalJSON() ([]byte, error) {
...
func (o *Opendataingestionruleversionresponse) UnmarshalJSON(b []byte) error {
	var OpendataingestionruleversionresponseMap map[string]interface{}
	err := json.Unmarshal(b, &OpendataingestionruleversionresponseMap)
	if err != nil {
		return err
	}
	
	if Id, ok := OpendataingestionruleversionresponseMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := OpendataingestionruleversionresponseMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Description, ok := OpendataingestionruleversionresponseMap["description"].(string); ok {
		o.Description = &Description
	}
    
	if Status, ok := OpendataingestionruleversionresponseMap["status"].(string); ok {
		o.Status = &Status
	}
    
	if Version, ok := OpendataingestionruleversionresponseMap["version"].(float64); ok {
		VersionInt := int(Version)
		o.Version = &VersionInt
	}
	
	if dateCreatedString, ok := OpendataingestionruleversionresponseMap["dateCreated"].(string); ok {
		DateCreated, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCreatedString)
		o.DateCreated = &DateCreated
	}
	
	if dateModifiedString, ok := OpendataingestionruleversionresponseMap["dateModified"].(string); ok {
		DateModified, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateModifiedString)
		o.DateModified = &DateModified
	}
	
	if Platform, ok := OpendataingestionruleversionresponseMap["platform"].(string); ok {
		o.Platform = &Platform
	}
    
	if IngestionRuleInfo, ok := OpendataingestionruleversionresponseMap["ingestionRuleInfo"].(map[string]interface{}); ok {
		IngestionRuleInfoString, _ := json.Marshal(IngestionRuleInfo)
		json.Unmarshal(IngestionRuleInfoString, &o.IngestionRuleInfo)
	}
	
	if Countries, ok := OpendataingestionruleversionresponseMap["countries"].([]interface{}); ok {
		CountriesString, _ := json.Marshal(Countries)
		json.Unmarshal(CountriesString, &o.Countries)
	}
	
	if ExternalSource, ok := OpendataingestionruleversionresponseMap["externalSource"].(map[string]interface{}); ok {
		ExternalSourceString, _ := json.Marshal(ExternalSource)
		json.Unmarshal(ExternalSourceString, &o.ExternalSource)
	}
	
	if SelfUri, ok := OpendataingestionruleversionresponseMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

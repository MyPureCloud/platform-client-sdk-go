...
func (o *Butimeofflimitvaluerange) SetField(field string, fieldValue interface{}) {
...
func (o Butimeofflimitvaluerange) MarshalJSON() ([]byte, error) {
...
func (o *Butimeofflimitvaluerange) UnmarshalJSON(b []byte) error {
	var ButimeofflimitvaluerangeMap map[string]interface{}
	err := json.Unmarshal(b, &ButimeofflimitvaluerangeMap)
	if err != nil {
		return err
	}
	
	if Id, ok := ButimeofflimitvaluerangeMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if TimeOffLimit, ok := ButimeofflimitvaluerangeMap["timeOffLimit"].(map[string]interface{}); ok {
		TimeOffLimitString, _ := json.Marshal(TimeOffLimit)
		json.Unmarshal(TimeOffLimitString, &o.TimeOffLimit)
	}
	
	if startDateString, ok := ButimeofflimitvaluerangeMap["startDate"].(string); ok {
		StartDate, _ := time.Parse("2006-01-02", startDateString)
		o.StartDate = &StartDate
	}
	
	if Granularity, ok := ButimeofflimitvaluerangeMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if LimitMinutesPerInterval, ok := ButimeofflimitvaluerangeMap["limitMinutesPerInterval"].([]interface{}); ok {
	...
	if AllocatedMinutesPerInterval, ok := ButimeofflimitvaluerangeMap["allocatedMinutesPerInterval"].([]interface{}); ok {
	...
	if WaitlistedMinutesPerInterval, ok := ButimeofflimitvaluerangeMap["waitlistedMinutesPerInterval"].([]interface{}); ok {
	...
	if WaitlistedRequestsPerInterval, ok := ButimeofflimitvaluerangeMap["waitlistedRequestsPerInterval"].([]interface{}); ok {
	...
	if Metadata, ok := ButimeofflimitvaluerangeMap["metadata"].(map[string]interface{}); ok {
		MetadataString, _ := json.Marshal(Metadata)
		json.Unmarshal(MetadataString, &o.Metadata)
	}
	
	if SelfUri, ok := ButimeofflimitvaluerangeMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

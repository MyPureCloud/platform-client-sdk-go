...
func (o *Learningscheduleslotsjobrequest) SetField(field string, fieldValue interface{}) {
...
func (o Learningscheduleslotsjobrequest) MarshalJSON() ([]byte, error) {
...
func (o *Learningscheduleslotsjobrequest) UnmarshalJSON(b []byte) error {
	var LearningscheduleslotsjobrequestMap map[string]interface{}
	err := json.Unmarshal(b, &LearningscheduleslotsjobrequestMap)
	if err != nil {
		return err
	}
	
	if UserIds, ok := LearningscheduleslotsjobrequestMap["userIds"].([]interface{}); ok {
		UserIdsString, _ := json.Marshal(UserIds)
		json.Unmarshal(UserIdsString, &o.UserIds)
	}
	
	if LengthInMinutes, ok := LearningscheduleslotsjobrequestMap["lengthInMinutes"].(float64); ok {
		LengthInMinutesInt := int(LengthInMinutes)
		o.LengthInMinutes = &LengthInMinutesInt
	}
	
	if ActivityCodeId, ok := LearningscheduleslotsjobrequestMap["activityCodeId"].(string); ok {
		o.ActivityCodeId = &ActivityCodeId
	}
    
	if Intervals, ok := LearningscheduleslotsjobrequestMap["intervals"].([]interface{}); ok {
		IntervalsString, _ := json.Marshal(Intervals)
		json.Unmarshal(IntervalsString, &o.Intervals)
	}
	
	if SlotsType, ok := LearningscheduleslotsjobrequestMap["slotsType"].(string); ok {

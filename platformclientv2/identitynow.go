...
func (o *Identitynow) SetField(field string, fieldValue interface{}) {
...
func (o *Identitynow) UnmarshalJSON(b []byte) error {
	...
	if LogoImageData, ok := IdentitynowMap["logoImageData"].(string); ok {
		o.LogoImageData = &LogoImageData
	}
    
	if NameIdentifierFormat, ok := IdentitynowMap["nameIdentifierFormat"].(string); ok {
		o.NameIdentifierFormat = &NameIdentifierFormat
	}
    
	if SsoBinding, ok := IdentitynowMap["ssoBinding"].(string); ok {
		o.SsoBinding = &SsoBinding
	}
    
	if SignAuthnRequests, ok := IdentitynowMap["signAuthnRequests"].(bool); ok {
		o.SignAuthnRequests = &SignAuthnRequests
	}
    
	if ProviderName, ok := IdentitynowMap["providerName"].(string); ok {
		o.ProviderName = &ProviderName
	}
    
	if DisplayOnLogin, ok := IdentitynowMap["displayOnLogin"].(bool); ok {
		o.DisplayOnLogin = &DisplayOnLogin
	}
    
	if MetadataURL, ok := IdentitynowMap["metadataURL"].(string); ok {
		o.MetadataURL = &MetadataURL
	}
    
	if ForceAuthn, ok := IdentitynowMap["forceAuthn"].(bool); ok {

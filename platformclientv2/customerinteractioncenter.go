...
func (o *Customerinteractioncenter) UnmarshalJSON(b []byte) error {
	...
	if LogoImageData, ok := CustomerinteractioncenterMap["logoImageData"].(string); ok {
		o.LogoImageData = &LogoImageData
	}
    
	if NameIdentifierFormat, ok := CustomerinteractioncenterMap["nameIdentifierFormat"].(string); ok {
		o.NameIdentifierFormat = &NameIdentifierFormat
	}
    
	if SsoBinding, ok := CustomerinteractioncenterMap["ssoBinding"].(string); ok {
		o.SsoBinding = &SsoBinding
	}
    
	if SignAuthnRequests, ok := CustomerinteractioncenterMap["signAuthnRequests"].(bool); ok {
		o.SignAuthnRequests = &SignAuthnRequests
	}
    
	if ProviderName, ok := CustomerinteractioncenterMap["providerName"].(string); ok {
		o.ProviderName = &ProviderName
	}
    
	if DisplayOnLogin, ok := CustomerinteractioncenterMap["displayOnLogin"].(bool); ok {
		o.DisplayOnLogin = &DisplayOnLogin
	}
    
	if MetadataURL, ok := CustomerinteractioncenterMap["metadataURL"].(string); ok {
		o.MetadataURL = &MetadataURL
	}
    
	if ForceAuthn, ok := CustomerinteractioncenterMap["forceAuthn"].(bool); ok {

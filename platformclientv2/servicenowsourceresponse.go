...
type Servicenowsourceresponse struct { 
	...
	Settings *Servicenowsettings `json:"settings,omitempty"`
...
func (o *Servicenowsourceresponse) SetField(field string, fieldValue interface{}) {
...
func (o Servicenowsourceresponse) MarshalJSON() ([]byte, error) {
...
func (o *Servicenowsourceresponse) UnmarshalJSON(b []byte) error {
	var ServicenowsourceresponseMap map[string]interface{}
	err := json.Unmarshal(b, &ServicenowsourceresponseMap)
	if err != nil {
		return err
	}
	
	if Id, ok := ServicenowsourceresponseMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := ServicenowsourceresponseMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if dateCreatedString, ok := ServicenowsourceresponseMap["dateCreated"].(string); ok {
		DateCreated, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCreatedString)
		o.DateCreated = &DateCreated
	}
	
	if dateModifiedString, ok := ServicenowsourceresponseMap["dateModified"].(string); ok {
		DateModified, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateModifiedString)
		o.DateModified = &DateModified
	}
	
	if VarType, ok := ServicenowsourceresponseMap["type"].(string); ok {
		o.VarType = &VarType
	}
    
	if Integration, ok := ServicenowsourceresponseMap["integration"].(map[string]interface{}); ok {
		IntegrationString, _ := json.Marshal(Integration)
		json.Unmarshal(IntegrationString, &o.Integration)
	}
	
	if SchedulePeriod, ok := ServicenowsourceresponseMap["schedulePeriod"].(float64); ok {
		SchedulePeriodInt := int(SchedulePeriod)
		o.SchedulePeriod = &SchedulePeriodInt
	}
	
	if LastSync, ok := ServicenowsourceresponseMap["lastSync"].(map[string]interface{}); ok {
		LastSyncString, _ := json.Marshal(LastSync)
		json.Unmarshal(LastSyncString, &o.LastSync)
	}
	
	if Settings, ok := ServicenowsourceresponseMap["settings"].(map[string]interface{}); ok {
		SettingsString, _ := json.Marshal(Settings)
		json.Unmarshal(SettingsString, &o.Settings)
	}
	
	if SelfUri, ok := ServicenowsourceresponseMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

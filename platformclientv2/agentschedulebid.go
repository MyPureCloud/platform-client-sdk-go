...
func (o *Agentschedulebid) SetField(field string, fieldValue interface{}) {
...
func (o Agentschedulebid) MarshalJSON() ([]byte, error) {
...
func (o *Agentschedulebid) UnmarshalJSON(b []byte) error {
	var AgentschedulebidMap map[string]interface{}
	err := json.Unmarshal(b, &AgentschedulebidMap)
	if err != nil {
		return err
	}
	
	if Id, ok := AgentschedulebidMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := AgentschedulebidMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if bidWindowStartDateString, ok := AgentschedulebidMap["bidWindowStartDate"].(string); ok {
		BidWindowStartDate, _ := time.Parse("2006-01-02", bidWindowStartDateString)
		o.BidWindowStartDate = &BidWindowStartDate
	}
	
	if bidWindowEndDateString, ok := AgentschedulebidMap["bidWindowEndDate"].(string); ok {
		BidWindowEndDate, _ := time.Parse("2006-01-02", bidWindowEndDateString)
		o.BidWindowEndDate = &BidWindowEndDate
	}
	
	if effectiveDateString, ok := AgentschedulebidMap["effectiveDate"].(string); ok {
		EffectiveDate, _ := time.Parse("2006-01-02", effectiveDateString)
		o.EffectiveDate = &EffectiveDate
	}
	
	if Status, ok := AgentschedulebidMap["status"].(string); ok {
		o.Status = &Status
	}
    
	if BidType, ok := AgentschedulebidMap["bidType"].(string); ok {
		o.BidType = &BidType
	}
    
	if SelfUri, ok := AgentschedulebidMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

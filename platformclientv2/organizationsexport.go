...
type Organizationsexport struct { 
	...
	QueryConditions *Organizationsexportqueryconditions `json:"queryConditions,omitempty"`
...
func (o *Organizationsexport) SetField(field string, fieldValue interface{}) {
...
func (o Organizationsexport) MarshalJSON() ([]byte, error) {
...
func (o *Organizationsexport) UnmarshalJSON(b []byte) error {
	var OrganizationsexportMap map[string]interface{}
	err := json.Unmarshal(b, &OrganizationsexportMap)
	if err != nil {
		return err
	}
	
	if Id, ok := OrganizationsexportMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if DivisionIds, ok := OrganizationsexportMap["divisionIds"].([]interface{}); ok {
		DivisionIdsString, _ := json.Marshal(DivisionIds)
		json.Unmarshal(DivisionIdsString, &o.DivisionIds)
	}
	
	if CreatedBy, ok := OrganizationsexportMap["createdBy"].(map[string]interface{}); ok {
		CreatedByString, _ := json.Marshal(CreatedBy)
		json.Unmarshal(CreatedByString, &o.CreatedBy)
	}
	
	if dateCreatedString, ok := OrganizationsexportMap["dateCreated"].(string); ok {
		DateCreated, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCreatedString)
		o.DateCreated = &DateCreated
	}
	
	if dateCompletionString, ok := OrganizationsexportMap["dateCompletion"].(string); ok {
		DateCompletion, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCompletionString)
		o.DateCompletion = &DateCompletion
	}
	
	if Status, ok := OrganizationsexportMap["status"].(string); ok {
		o.Status = &Status
	}
    
	if DownloadUrl, ok := OrganizationsexportMap["downloadUrl"].(string); ok {
		o.DownloadUrl = &DownloadUrl
	}
    
	if ResultRowCount, ok := OrganizationsexportMap["resultRowCount"].(float64); ok {
		ResultRowCountInt := int(ResultRowCount)
		o.ResultRowCount = &ResultRowCountInt
	}
	
	if QueryConditions, ok := OrganizationsexportMap["queryConditions"].(map[string]interface{}); ok {
		QueryConditionsString, _ := json.Marshal(QueryConditions)
		json.Unmarshal(QueryConditionsString, &o.QueryConditions)
	}
	
	if SelfUri, ok := OrganizationsexportMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

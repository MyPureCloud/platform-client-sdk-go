...
type Contentdatepicker struct { 
	...
	AvailableTimes *[]Contentdatepickeravailabletime `json:"availableTimes,omitempty"`
}

// SetField uses reflection to set a field on the model if the model has a property SetFieldNames, and triggers custom JSON serialization logic to only serialize properties that have been set using this function.
func (o *Contentdatepicker) SetField(field string, fieldValue interface{}) {
...
func (o Contentdatepicker) MarshalJSON() ([]byte, error) {
...
func (o *Contentdatepicker) UnmarshalJSON(b []byte) error {
	var ContentdatepickerMap map[string]interface{}
	err := json.Unmarshal(b, &ContentdatepickerMap)
	if err != nil {
		return err
	}
	
	if Title, ok := ContentdatepickerMap["title"].(string); ok {
		o.Title = &Title
	}
    
	if Subtitle, ok := ContentdatepickerMap["subtitle"].(string); ok {
		o.Subtitle = &Subtitle
	}
    
	if ImageUrl, ok := ContentdatepickerMap["imageUrl"].(string); ok {
		o.ImageUrl = &ImageUrl
	}
    
	if dateMinimumString, ok := ContentdatepickerMap["dateMinimum"].(string); ok {
		DateMinimum, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateMinimumString)
		o.DateMinimum = &DateMinimum
	}
	
	if dateMaximumString, ok := ContentdatepickerMap["dateMaximum"].(string); ok {
		DateMaximum, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateMaximumString)
		o.DateMaximum = &DateMaximum
	}
	
	if AvailableTimes, ok := ContentdatepickerMap["availableTimes"].([]interface{}); ok {

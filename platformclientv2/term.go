...
func (o *Term) SetField(field string, fieldValue interface{}) {
...
func (o *Term) UnmarshalJSON(b []byte) error {
	var TermMap map[string]interface{}
	err := json.Unmarshal(b, &TermMap)
	if err != nil {
		return err
	}
	
	if Word, ok := TermMap["word"].(string); ok {
		o.Word = &Word
	}
    
	if ParticipantType, ok := TermMap["participantType"].(string); ok {

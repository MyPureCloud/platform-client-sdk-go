...
func (o *Emailroutingtransferevent) SetField(field string, fieldValue interface{}) {
...
func (o Emailroutingtransferevent) MarshalJSON() ([]byte, error) {
...
func (o *Emailroutingtransferevent) UnmarshalJSON(b []byte) error {
	var EmailroutingtransfereventMap map[string]interface{}
	err := json.Unmarshal(b, &EmailroutingtransfereventMap)
	if err != nil {
		return err
	}
	
	if EventId, ok := EmailroutingtransfereventMap["eventId"].(string); ok {
		o.EventId = &EventId
	}
    
	if eventDateTimeString, ok := EmailroutingtransfereventMap["eventDateTime"].(string); ok {
		EventDateTime, _ := time.Parse("2006-01-02T15:04:05.999999Z", eventDateTimeString)
		o.EventDateTime = &EventDateTime
	}
	
	if ConversationId, ok := EmailroutingtransfereventMap["conversationId"].(string); ok {
		o.ConversationId = &ConversationId
	}
    
	if TransferType, ok := EmailroutingtransfereventMap["transferType"].(string); ok {
		o.TransferType = &TransferType
	}
    
	if CommandId, ok := EmailroutingtransfereventMap["commandId"].(string); ok {
		o.CommandId = &CommandId
	}
    
	if InitiatingCommunicationId, ok := EmailroutingtransfereventMap["initiatingCommunicationId"].(string); ok {
		o.InitiatingCommunicationId = &InitiatingCommunicationId
	}
    
	if TargetCommunicationId, ok := EmailroutingtransfereventMap["targetCommunicationId"].(string); ok {
		o.TargetCommunicationId = &TargetCommunicationId
	}
    
	if ObjectCommunicationId, ok := EmailroutingtransfereventMap["objectCommunicationId"].(string); ok {
		o.ObjectCommunicationId = &ObjectCommunicationId
	}
    
	if DestinationQueueId, ok := EmailroutingtransfereventMap["destinationQueueId"].(string); ok {
		o.DestinationQueueId = &DestinationQueueId
	}
    
	if LanguageId, ok := EmailroutingtransfereventMap["languageId"].(string); ok {
		o.LanguageId = &LanguageId
	}
    
	if SkillIds, ok := EmailroutingtransfereventMap["skillIds"].([]interface{}); ok {

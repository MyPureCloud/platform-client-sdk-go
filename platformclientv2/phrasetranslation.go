...
func (o *Phrasetranslation) SetField(field string, fieldValue interface{}) {
...
func (o Phrasetranslation) MarshalJSON() ([]byte, error) {
...
func (o *Phrasetranslation) UnmarshalJSON(b []byte) error {
	var PhrasetranslationMap map[string]interface{}
	err := json.Unmarshal(b, &PhrasetranslationMap)
	if err != nil {
		return err
	}
	
	if StartTimeMs, ok := PhrasetranslationMap["startTimeMs"].(float64); ok {
		StartTimeMsInt := int(StartTimeMs)
		o.StartTimeMs = &StartTimeMsInt
	}
	
	if ParticipantPurpose, ok := PhrasetranslationMap["participantPurpose"].(string); ok {
		o.ParticipantPurpose = &ParticipantPurpose
	}
    
	if TranslatedText, ok := PhrasetranslationMap["translatedText"].(string); ok {
		o.TranslatedText = &TranslatedText
	}

...
func (o *Mediasettings) SetField(field string, fieldValue interface{}) {
...
func (o Mediasettings) MarshalJSON() ([]byte, error) {
...
func (o *Mediasettings) UnmarshalJSON(b []byte) error {
	var MediasettingsMap map[string]interface{}
	err := json.Unmarshal(b, &MediasettingsMap)
	if err != nil {
		return err
	}
	
	if EnableAutoAnswer, ok := MediasettingsMap["enableAutoAnswer"].(bool); ok {
		o.EnableAutoAnswer = &EnableAutoAnswer
	}
    
	if AlertingTimeoutSeconds, ok := MediasettingsMap["alertingTimeoutSeconds"].(float64); ok {
		AlertingTimeoutSecondsInt := int(AlertingTimeoutSeconds)
		o.AlertingTimeoutSeconds = &AlertingTimeoutSecondsInt
	}
	
	if ServiceLevel, ok := MediasettingsMap["serviceLevel"].(map[string]interface{}); ok {

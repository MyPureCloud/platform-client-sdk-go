...
type Internalmessageconversation struct { 
	...
	Participants *[]Internalmessagemediaparticipant `json:"participants,omitempty"`
...
func (o *Internalmessageconversation) SetField(field string, fieldValue interface{}) {
...
func (o Internalmessageconversation) MarshalJSON() ([]byte, error) {
...
func (o *Internalmessageconversation) UnmarshalJSON(b []byte) error {
	var InternalmessageconversationMap map[string]interface{}
	err := json.Unmarshal(b, &InternalmessageconversationMap)
	if err != nil {
		return err
	}
	
	if Id, ok := InternalmessageconversationMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := InternalmessageconversationMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Participants, ok := InternalmessageconversationMap["participants"].([]interface{}); ok {
		ParticipantsString, _ := json.Marshal(Participants)
		json.Unmarshal(ParticipantsString, &o.Participants)
	}
	
	if OtherMediaUris, ok := InternalmessageconversationMap["otherMediaUris"].([]interface{}); ok {
		OtherMediaUrisString, _ := json.Marshal(OtherMediaUris)
		json.Unmarshal(OtherMediaUrisString, &o.OtherMediaUris)
	}
	
	if RecentTransfers, ok := InternalmessageconversationMap["recentTransfers"].([]interface{}); ok {
		RecentTransfersString, _ := json.Marshal(RecentTransfers)
		json.Unmarshal(RecentTransfersString, &o.RecentTransfers)
	}
	
	if UtilizationLabelId, ok := InternalmessageconversationMap["utilizationLabelId"].(string); ok {
		o.UtilizationLabelId = &UtilizationLabelId
	}
    
	if inactivityTimeoutString, ok := InternalmessageconversationMap["inactivityTimeout"].(string); ok {
		InactivityTimeout, _ := time.Parse("2006-01-02T15:04:05.999999Z", inactivityTimeoutString)
		o.InactivityTimeout = &InactivityTimeout
	}
	
	if Divisions, ok := InternalmessageconversationMap["divisions"].([]interface{}); ok {
		DivisionsString, _ := json.Marshal(Divisions)
		json.Unmarshal(DivisionsString, &o.Divisions)
	}
	
	if SelfUri, ok := InternalmessageconversationMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

...
func (o *Onelogin) SetField(field string, fieldValue interface{}) {
...
func (o *Onelogin) UnmarshalJSON(b []byte) error {
	...
	if LogoImageData, ok := OneloginMap["logoImageData"].(string); ok {
		o.LogoImageData = &LogoImageData
	}
    
	if NameIdentifierFormat, ok := OneloginMap["nameIdentifierFormat"].(string); ok {
		o.NameIdentifierFormat = &NameIdentifierFormat
	}
    
	if SsoBinding, ok := OneloginMap["ssoBinding"].(string); ok {
		o.SsoBinding = &SsoBinding
	}
    
	if SignAuthnRequests, ok := OneloginMap["signAuthnRequests"].(bool); ok {
		o.SignAuthnRequests = &SignAuthnRequests
	}
    
	if ProviderName, ok := OneloginMap["providerName"].(string); ok {
		o.ProviderName = &ProviderName
	}
    
	if DisplayOnLogin, ok := OneloginMap["displayOnLogin"].(bool); ok {
		o.DisplayOnLogin = &DisplayOnLogin
	}
    
	if MetadataURL, ok := OneloginMap["metadataURL"].(string); ok {
		o.MetadataURL = &MetadataURL
	}
    
	if ForceAuthn, ok := OneloginMap["forceAuthn"].(bool); ok {

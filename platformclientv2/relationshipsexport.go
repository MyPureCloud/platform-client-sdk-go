...
type Relationshipsexport struct { 
	...
	QueryConditions *Relationshipsexportqueryconditions `json:"queryConditions,omitempty"`
...
func (o *Relationshipsexport) SetField(field string, fieldValue interface{}) {
...
func (o Relationshipsexport) MarshalJSON() ([]byte, error) {
...
func (o *Relationshipsexport) UnmarshalJSON(b []byte) error {
	var RelationshipsexportMap map[string]interface{}
	err := json.Unmarshal(b, &RelationshipsexportMap)
	if err != nil {
		return err
	}
	
	if Id, ok := RelationshipsexportMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if DivisionIds, ok := RelationshipsexportMap["divisionIds"].([]interface{}); ok {
		DivisionIdsString, _ := json.Marshal(DivisionIds)
		json.Unmarshal(DivisionIdsString, &o.DivisionIds)
	}
	
	if CreatedBy, ok := RelationshipsexportMap["createdBy"].(map[string]interface{}); ok {
		CreatedByString, _ := json.Marshal(CreatedBy)
		json.Unmarshal(CreatedByString, &o.CreatedBy)
	}
	
	if dateCreatedString, ok := RelationshipsexportMap["dateCreated"].(string); ok {
		DateCreated, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCreatedString)
		o.DateCreated = &DateCreated
	}
	
	if dateCompletionString, ok := RelationshipsexportMap["dateCompletion"].(string); ok {
		DateCompletion, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCompletionString)
		o.DateCompletion = &DateCompletion
	}
	
	if Status, ok := RelationshipsexportMap["status"].(string); ok {
		o.Status = &Status
	}
    
	if DownloadUrl, ok := RelationshipsexportMap["downloadUrl"].(string); ok {
		o.DownloadUrl = &DownloadUrl
	}
    
	if ResultRowCount, ok := RelationshipsexportMap["resultRowCount"].(float64); ok {
		ResultRowCountInt := int(ResultRowCount)
		o.ResultRowCount = &ResultRowCountInt
	}
	
	if QueryConditions, ok := RelationshipsexportMap["queryConditions"].(map[string]interface{}); ok {
		QueryConditionsString, _ := json.Marshal(QueryConditions)
		json.Unmarshal(QueryConditionsString, &o.QueryConditions)
	}
	
	if SelfUri, ok := RelationshipsexportMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

...
func (o *Businessrulesdataschema) SetField(field string, fieldValue interface{}) {
...
func (o Businessrulesdataschema) MarshalJSON() ([]byte, error) {
...
func (o *Businessrulesdataschema) UnmarshalJSON(b []byte) error {
	var BusinessrulesdataschemaMap map[string]interface{}
	err := json.Unmarshal(b, &BusinessrulesdataschemaMap)
	if err != nil {
		return err
	}
	
	if Id, ok := BusinessrulesdataschemaMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := BusinessrulesdataschemaMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if Version, ok := BusinessrulesdataschemaMap["version"].(float64); ok {
		VersionInt := int(Version)
		o.Version = &VersionInt
	}
	
	if AppliesTo, ok := BusinessrulesdataschemaMap["appliesTo"].([]interface{}); ok {
		AppliesToString, _ := json.Marshal(AppliesTo)
		json.Unmarshal(AppliesToString, &o.AppliesTo)
	}
	
	if Enabled, ok := BusinessrulesdataschemaMap["enabled"].(bool); ok {
		o.Enabled = &Enabled
	}
    
	if CreatedBy, ok := BusinessrulesdataschemaMap["createdBy"].(map[string]interface{}); ok {
		CreatedByString, _ := json.Marshal(CreatedBy)
		json.Unmarshal(CreatedByString, &o.CreatedBy)
	}
	
	if dateCreatedString, ok := BusinessrulesdataschemaMap["dateCreated"].(string); ok {
		DateCreated, _ := time.Parse("2006-01-02T15:04:05.999999Z", dateCreatedString)
		o.DateCreated = &DateCreated
	}
	
	if JsonSchema, ok := BusinessrulesdataschemaMap["jsonSchema"].(map[string]interface{}); ok {
		JsonSchemaString, _ := json.Marshal(JsonSchema)
		json.Unmarshal(JsonSchemaString, &o.JsonSchema)
	}
	
	if SelfUri, ok := BusinessrulesdataschemaMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

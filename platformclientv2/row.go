...
func (o *Row) SetField(field string, fieldValue interface{}) {
...
func (o *Row) UnmarshalJSON(b []byte) error {
	var RowMap map[string]interface{}
	err := json.Unmarshal(b, &RowMap)
	if err != nil {
		return err
	}
	
	if RowId, ok := RowMap["rowId"].(string); ok {
		o.RowId = &RowId
	}
    
	if Inputs, ok := RowMap["inputs"].(map[string]interface{}); ok {
		InputsString, _ := json.Marshal(Inputs)
		json.Unmarshal(InputsString, &o.Inputs)
	}
	
	if Outputs, ok := RowMap["outputs"].(map[string]interface{}); ok {

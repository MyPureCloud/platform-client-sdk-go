...
type Decisionmetricsuploadresponse struct { 
	...
	UploadBodySchema *Decisionmetricsuploadschema `json:"uploadBodySchema,omitempty"`
}

// SetField uses reflection to set a field on the model if the model has a property SetFieldNames, and triggers custom JSON serialization logic to only serialize properties that have been set using this function.
func (o *Decisionmetricsuploadresponse) SetField(field string, fieldValue interface{}) {
...
func (o Decisionmetricsuploadresponse) MarshalJSON() ([]byte, error) {
...
func (o *Decisionmetricsuploadresponse) UnmarshalJSON(b []byte) error {
	var DecisionmetricsuploadresponseMap map[string]interface{}
	err := json.Unmarshal(b, &DecisionmetricsuploadresponseMap)
	if err != nil {
		return err
	}
	
	if UploadKey, ok := DecisionmetricsuploadresponseMap["uploadKey"].(string); ok {
		o.UploadKey = &UploadKey
	}
    
	if Url, ok := DecisionmetricsuploadresponseMap["url"].(string); ok {
		o.Url = &Url
	}
    
	if Headers, ok := DecisionmetricsuploadresponseMap["headers"].(map[string]interface{}); ok {
		HeadersString, _ := json.Marshal(Headers)
		json.Unmarshal(HeadersString, &o.Headers)
	}
	
	if UploadBodySchema, ok := DecisionmetricsuploadresponseMap["uploadBodySchema"].(map[string]interface{}); ok {

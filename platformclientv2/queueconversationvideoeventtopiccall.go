...
type Queueconversationvideoeventtopiccall struct { 
	...
	// RecordersState
	RecordersState *Queueconversationvideoeventtopicrecordersstate `json:"recordersState,omitempty"`
	...
	// Disposition
	Disposition *Queueconversationvideoeventtopicdisposition `json:"disposition,omitempty"`
...
func (o *Queueconversationvideoeventtopiccall) UnmarshalJSON(b []byte) error {
	...
	if RecordersState, ok := QueueconversationvideoeventtopiccallMap["recordersState"].(map[string]interface{}); ok {
	...
	if QueueMediaSettings, ok := QueueconversationvideoeventtopiccallMap["queueMediaSettings"].(map[string]interface{}); ok {
		QueueMediaSettingsString, _ := json.Marshal(QueueMediaSettings)
		json.Unmarshal(QueueMediaSettingsString, &o.QueueMediaSettings)
	}
	
	if Disposition, ok := QueueconversationvideoeventtopiccallMap["disposition"].(map[string]interface{}); ok {
		DispositionString, _ := json.Marshal(Disposition)
		json.Unmarshal(DispositionString, &o.Disposition)
	}
	
	if TransferSource, ok := QueueconversationvideoeventtopiccallMap["transferSource"].(string); ok {
		o.TransferSource = &TransferSource
	}
    
	if PublicIpAddress, ok := QueueconversationvideoeventtopiccallMap["publicIpAddress"].(string); ok {

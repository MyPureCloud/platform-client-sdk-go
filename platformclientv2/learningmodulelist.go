...
func (o *Learningmodulelist) SetField(field string, fieldValue interface{}) {
...
func (o Learningmodulelist) MarshalJSON() ([]byte, error) {
...
func (o *Learningmodulelist) UnmarshalJSON(b []byte) error {
	var LearningmodulelistMap map[string]interface{}
	err := json.Unmarshal(b, &LearningmodulelistMap)
	if err != nil {
		return err
	}
	
	if Entities, ok := LearningmodulelistMap["entities"].([]interface{}); ok {
		EntitiesString, _ := json.Marshal(Entities)
		json.Unmarshal(EntitiesString, &o.Entities)
	}
	
	if PageSize, ok := LearningmodulelistMap["pageSize"].(float64); ok {
		PageSizeInt := int(PageSize)
		o.PageSize = &PageSizeInt
	}
	
	if PageNumber, ok := LearningmodulelistMap["pageNumber"].(float64); ok {
		PageNumberInt := int(PageNumber)
		o.PageNumber = &PageNumberInt
	}
	
	if Total, ok := LearningmodulelistMap["total"].(float64); ok {
		TotalInt := int(Total)
		o.Total = &TotalInt
	}
	
	if TotalLegacyRules, ok := LearningmodulelistMap["totalLegacyRules"].(float64); ok {
		TotalLegacyRulesInt := int(TotalLegacyRules)

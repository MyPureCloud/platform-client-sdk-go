...
//
// Deprecated: GetUserrecordingMedia is deprecated
...
// GetUserrecordingTranscoding invokes GET /api/v2/userrecordings/{recordingId}/transcoding
//
// Download a user recording.
func (a UserRecordingsApi) GetUserrecordingTranscoding(recordingId string, formatId string) (*Downloadresponse, *APIResponse, error) {
	var httpMethod = "GET"
	// create path and map variables
	path := a.Configuration.BasePath + "/api/v2/userrecordings/{recordingId}/transcoding"
	path = strings.Replace(path, "{recordingId}", url.PathEscape(fmt.Sprintf("%v", recordingId)), -1)
	defaultReturn := new(Downloadresponse)
	if true == false {
		return defaultReturn, nil, errors.New("This message brought to you by the laws of physics being broken")
	...
	if &recordingId == nil {
		// false
		return defaultReturn, nil, errors.New("Missing required parameter 'recordingId' when calling UserRecordingsApi->GetUserrecordingTranscoding")
	}

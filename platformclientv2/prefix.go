...
func (o *Prefix) SetField(field string, fieldValue interface{}) {
...
func (o *Prefix) UnmarshalJSON(b []byte) error {
	var PrefixMap map[string]interface{}
	err := json.Unmarshal(b, &PrefixMap)
	if err != nil {
		return err
	}
	
	if CountryCode, ok := PrefixMap["countryCode"].(string); ok {
		o.CountryCode = &CountryCode
	}
    
	if Number, ok := PrefixMap["number"].(string); ok {
		o.Number = &Number
	}
    
	if VarType, ok := PrefixMap["type"].(string); ok {
		o.VarType = &VarType
	}
    
	if Action, ok := PrefixMap["action"].(string); ok {
		o.Action = &Action
	}

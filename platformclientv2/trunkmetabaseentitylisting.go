...
func (o *Trunkmetabaseentitylisting) SetField(field string, fieldValue interface{}) {
...
func (o *Trunkmetabaseentitylisting) UnmarshalJSON(b []byte) error {
	var TrunkmetabaseentitylistingMap map[string]interface{}
	err := json.Unmarshal(b, &TrunkmetabaseentitylistingMap)
	if err != nil {
		return err
	}
	
	if Entities, ok := TrunkmetabaseentitylistingMap["entities"].([]interface{}); ok {
		EntitiesString, _ := json.Marshal(Entities)
		json.Unmarshal(EntitiesString, &o.Entities)
	}
	
	if PageSize, ok := TrunkmetabaseentitylistingMap["pageSize"].(float64); ok {
		PageSizeInt := int(PageSize)
		o.PageSize = &PageSizeInt
	}
	
	if PageNumber, ok := TrunkmetabaseentitylistingMap["pageNumber"].(float64); ok {
		PageNumberInt := int(PageNumber)
		o.PageNumber = &PageNumberInt
	}
	
	if Total, ok := TrunkmetabaseentitylistingMap["total"].(float64); ok {
		TotalInt := int(Total)
		o.Total = &TotalInt
	}
	
	if TotalNumberOfEntities, ok := TrunkmetabaseentitylistingMap["totalNumberOfEntities"].(float64); ok {
		TotalNumberOfEntitiesInt := int(TotalNumberOfEntities)
		o.TotalNumberOfEntities = &TotalNumberOfEntitiesInt
	}
	
	if FirstUri, ok := TrunkmetabaseentitylistingMap["firstUri"].(string); ok {
		o.FirstUri = &FirstUri
	}
    
	if NextUri, ok := TrunkmetabaseentitylistingMap["nextUri"].(string); ok {
		o.NextUri = &NextUri
	}
    
	if PreviousUri, ok := TrunkmetabaseentitylistingMap["previousUri"].(string); ok {

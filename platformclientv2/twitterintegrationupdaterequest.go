...
func (o *Twitterintegrationupdaterequest) SetField(field string, fieldValue interface{}) {
...
func (o Twitterintegrationupdaterequest) MarshalJSON() ([]byte, error) {
...
func (o *Twitterintegrationupdaterequest) UnmarshalJSON(b []byte) error {
	var TwitterintegrationupdaterequestMap map[string]interface{}
	err := json.Unmarshal(b, &TwitterintegrationupdaterequestMap)
	if err != nil {
		return err
	}
	
	if Id, ok := TwitterintegrationupdaterequestMap["id"].(string); ok {
		o.Id = &Id
	}
    
	if Name, ok := TwitterintegrationupdaterequestMap["name"].(string); ok {
		o.Name = &Name
	}
    
	if SupportedContent, ok := TwitterintegrationupdaterequestMap["supportedContent"].(map[string]interface{}); ok {
		SupportedContentString, _ := json.Marshal(SupportedContent)
		json.Unmarshal(SupportedContentString, &o.SupportedContent)
	}
	
	if MessagingSetting, ok := TwitterintegrationupdaterequestMap["messagingSetting"].(map[string]interface{}); ok {
		MessagingSettingString, _ := json.Marshal(MessagingSetting)
		json.Unmarshal(MessagingSettingString, &o.MessagingSetting)
	}
	
	if SignupCode, ok := TwitterintegrationupdaterequestMap["signupCode"].(string); ok {
		o.SignupCode = &SignupCode
	}
    
	if CodeChallenge, ok := TwitterintegrationupdaterequestMap["codeChallenge"].(string); ok {
		o.CodeChallenge = &CodeChallenge
	}
    
	if RedirectUri, ok := TwitterintegrationupdaterequestMap["redirectUri"].(string); ok {
		o.RedirectUri = &RedirectUri
	}
    
	if SelfUri, ok := TwitterintegrationupdaterequestMap["selfUri"].(string); ok {
		o.SelfUri = &SelfUri
	}

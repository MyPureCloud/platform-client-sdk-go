...
func (o *Teamactivityentitydata) SetField(field string, fieldValue interface{}) {
...
func (o Teamactivityentitydata) MarshalJSON() ([]byte, error) {
...
func (o *Teamactivityentitydata) UnmarshalJSON(b []byte) error {
	var TeamactivityentitydataMap map[string]interface{}
	err := json.Unmarshal(b, &TeamactivityentitydataMap)
	if err != nil {
		return err
	}
	
	if activityDateString, ok := TeamactivityentitydataMap["activityDate"].(string); ok {
		ActivityDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", activityDateString)
		o.ActivityDate = &ActivityDate
	}
	
	if OrganizationPresenceId, ok := TeamactivityentitydataMap["organizationPresenceId"].(string); ok {
		o.OrganizationPresenceId = &OrganizationPresenceId
	}
    
	if presenceDateString, ok := TeamactivityentitydataMap["presenceDate"].(string); ok {
		PresenceDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", presenceDateString)
		o.PresenceDate = &PresenceDate
	}
	
	if QueueId, ok := TeamactivityentitydataMap["queueId"].(string); ok {
		o.QueueId = &QueueId
	}
    
	if QueueMembershipStatus, ok := TeamactivityentitydataMap["queueMembershipStatus"].(string); ok {
		o.QueueMembershipStatus = &QueueMembershipStatus
	}
    
	if RoutingStatus, ok := TeamactivityentitydataMap["routingStatus"].(string); ok {
		o.RoutingStatus = &RoutingStatus
	}
    
	if routingStatusDateString, ok := TeamactivityentitydataMap["routingStatusDate"].(string); ok {
		RoutingStatusDate, _ := time.Parse("2006-01-02T15:04:05.999999Z", routingStatusDateString)
		o.RoutingStatusDate = &RoutingStatusDate
	}
	
	if SystemPresence, ok := TeamactivityentitydataMap["systemPresence"].(string); ok {
		o.SystemPresence = &SystemPresence
	}
    
	if TeamId, ok := TeamactivityentitydataMap["teamId"].(string); ok {
		o.TeamId = &TeamId
	}
    
	if UserId, ok := TeamactivityentitydataMap["userId"].(string); ok {
		o.UserId = &UserId
	}

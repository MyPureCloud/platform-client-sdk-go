...
func (o *Knowledgeaggregationquery) SetField(field string, fieldValue interface{}) {
...
func (o Knowledgeaggregationquery) MarshalJSON() ([]byte, error) {
...
func (o *Knowledgeaggregationquery) UnmarshalJSON(b []byte) error {
	var KnowledgeaggregationqueryMap map[string]interface{}
	err := json.Unmarshal(b, &KnowledgeaggregationqueryMap)
	if err != nil {
		return err
	}
	
	if Interval, ok := KnowledgeaggregationqueryMap["interval"].(string); ok {
		o.Interval = &Interval
	}
    
	if Granularity, ok := KnowledgeaggregationqueryMap["granularity"].(string); ok {
		o.Granularity = &Granularity
	}
    
	if TimeZone, ok := KnowledgeaggregationqueryMap["timeZone"].(string); ok {
		o.TimeZone = &TimeZone
	}
    
	if GroupBy, ok := KnowledgeaggregationqueryMap["groupBy"].([]interface{}); ok {
		GroupByString, _ := json.Marshal(GroupBy)
		json.Unmarshal(GroupByString, &o.GroupBy)
	}
	
	if Filter, ok := KnowledgeaggregationqueryMap["filter"].(map[string]interface{}); ok {
		FilterString, _ := json.Marshal(Filter)
		json.Unmarshal(FilterString, &o.Filter)
	}
	
	if Metrics, ok := KnowledgeaggregationqueryMap["metrics"].([]interface{}); ok {
		MetricsString, _ := json.Marshal(Metrics)
		json.Unmarshal(MetricsString, &o.Metrics)
	}
	
	if FlattenMultivaluedDimensions, ok := KnowledgeaggregationqueryMap["flattenMultivaluedDimensions"].(bool); ok {
		o.FlattenMultivaluedDimensions = &FlattenMultivaluedDimensions
	}
    
	if Views, ok := KnowledgeaggregationqueryMap["views"].([]interface{}); ok {
		ViewsString, _ := json.Marshal(Views)
		json.Unmarshal(ViewsString, &o.Views)
	}
	
	if AlternateTimeDimension, ok := KnowledgeaggregationqueryMap["alternateTimeDimension"].(string); ok {
		o.AlternateTimeDimension = &AlternateTimeDimension
	}
    
	if QueryType, ok := KnowledgeaggregationqueryMap["queryType"].(string); ok {
		o.QueryType = &QueryType
	}
    
	if SortMetric, ok := KnowledgeaggregationqueryMap["sortMetric"].(map[string]interface{}); ok {
		SortMetricString, _ := json.Marshal(SortMetric)
		json.Unmarshal(SortMetricString, &o.SortMetric)
	}
	
	if Limit, ok := KnowledgeaggregationqueryMap["limit"].(float64); ok {
		LimitInt := int(Limit)
		o.Limit = &LimitInt
	}

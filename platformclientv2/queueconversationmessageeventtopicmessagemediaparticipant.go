...
type Queueconversationmessageeventtopicmessagemediaparticipant struct { 
	...
	// QueueMediaSettings
	QueueMediaSettings *Queueconversationmessageeventtopicqueuemediasettings `json:"queueMediaSettings,omitempty"`
...
func (o *Queueconversationmessageeventtopicmessagemediaparticipant) UnmarshalJSON(b []byte) error {
	...
	if ExternalContactInitialDivisionId, ok := QueueconversationmessageeventtopicmessagemediaparticipantMap["externalContactInitialDivisionId"].(string); ok {
	...
	if resumeTimeString, ok := QueueconversationmessageeventtopicmessagemediaparticipantMap["resumeTime"].(string); ok {
		ResumeTime, _ := time.Parse("2006-01-02T15:04:05.999999Z", resumeTimeString)
		o.ResumeTime = &ResumeTime
	}
	
	if parkTimeString, ok := QueueconversationmessageeventtopicmessagemediaparticipantMap["parkTime"].(string); ok {
		ParkTime, _ := time.Parse("2006-01-02T15:04:05.999999Z", parkTimeString)
		o.ParkTime = &ParkTime
	}
	
	if MediaRoles, ok := QueueconversationmessageeventtopicmessagemediaparticipantMap["mediaRoles"].([]interface{}); ok {
		MediaRolesString, _ := json.Marshal(MediaRoles)
		json.Unmarshal(MediaRolesString, &o.MediaRoles)
	}
	
	if QueueMediaSettings, ok := QueueconversationmessageeventtopicmessagemediaparticipantMap["queueMediaSettings"].(map[string]interface{}); ok {
